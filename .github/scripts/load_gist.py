...
def filter_payload_for_env(payload: dict[str, Any]) -> None:
    """浏览器侧 paper-hide.ts 写入的 hiddenPapers 字段不应进 CI $GITHUB_ENV。
    这是 paper-hide 时期的旧字段,2022 年初还在用;如下游 CI 误读会序列化
    成 `hiddenPapers=["2401.01234","2405.05678"]` 数组值,不会被认定为 secret,
    但仍然是我们不需要的字段。

    阶段 2 之前还 pop 过 userLibrary / schemaVersion(那时有过用户图书馆
    Gist 同步),那条路已取消,只剩下 hiddenPapers 这一项。"""
