...
- **2026-09-10** 🔎 `--web-search` general web search 工具（iter #68）：关闭与 Sakana AI Scientist v2 / STORM / OpenAI Deep Research "tool use" 的第二大短板（第一是 iter #56 `--search-arxiv`）。默认 **stub 模式**（零依赖零网络，返回 0 结果，不报错 — 让本地 dry-run 仍能跑通），有 `WEB_SEARCH_API_KEY` 环境变量时自动走 **Tavily backend**（`https://api.tavily.com/search`，AI 友好的语义 + 关键词混合搜索 + source ranking）。支持过滤：`--include-domain D` / `--exclude-domain D`（可重复，含 subdomain 匹配）/`--web-max N`（默认 5）/ `--min-score N`（0-1，丢低质量命中）/ `--web-backend stub|tavily`（显式覆盖）/`--json` 出机器可读。新模块 `astro-src/lib/agents/web-search.{mjs,ts}`（与 export-bundle / paper-compiler / synthesis-pdf / synthesis-diff 同 Phase A shim 双 surface 共享模式）暴露 8 个纯函数：`normalizeWebSearchUrl` 去 utm_*/fbclid/gclid/ref_*/hsa_*/__hs*/mc_*/_ga/_gl/_gid/oly_*/vero_*/trk/ncid/icid/src 等 tracking params + 强制 https + 去 fragment + 去非 root 尾 slash（同一结果因 utm 不同出现多次 → dedupe 兜得住）+ `stubSearch` + `buildTavilyRequest` 拼 POST body + `parseTavilyResponse` strip html 标签 + 截 500 字符 snippet + 抽 source domain + score + published_date + `dedupeWebSearchResults` 按 normalized url 去重（score max 合并）+ `filterWebSearchResults`（includeDomains/excludeDomains/minScore；没 score 字段的结果总是保留）+ `searchWeb` 主入口 stub/tavily 路由 + `formatWebSearchText` CLI 渲染（score + publishedAt + snippet 截 200 字符）。CLI 新 flag `--web-search "<QUERY>"` + 5 个修饰 flag。42 个新 lib 单测覆盖 normalize 9（utm/fbclid/gclid/ref_*/https upgrade/trailing slash/fragment/empty/无效 URL/非 tracking 保留）+ stub/searchWeb 5（stub 默认/空 query/tavily 路由/字节稳定）+ buildTavilyRequest 5（默认 max_results=5/search_depth=basic/自定义 max/depth/include+exclude domains/空 api_key）+ parseTavilyResponse 6（normalize/title fallback/非数组结果/500 字符截断/source 抽域名/strip html 标签）+ dedupe 4（normalized URL 去重 + score max/不同 URL 保留/空 + null/无 url 跳过）+ filter 6（includeDomains/subdomain 匹配/excludeDomains/minScore/组合/空 opts）+ 字节稳定 + format 7。新关闭 `docs/agents-workflow.md §6` "加 web search 工具" 候选。详见 `## 🤖 多智能体科研自动化` → `### 5.1 上手路径`。
- **2026-09-10** 📐 LnCS / AAAI / IJCAI 模板加到 `PAPER_LATEX_TEMPLATES`（iter #67）：iter #65 注册表从 6 个扩到 9 个 — 加 `llncs`（Springer Lecture Notes in Computer Science / llncs.cls，作者指南从 springer.com 下载）+ `aaai`（AAAI / aaai24.sty 占位，年份可改，Author Kit 从 aaai.org 下载）+ `ijcai`（IJCAI / ijcai24.sty 占位，年份可改，从 ijcai.org 下载）。每个模板都遵循 iter #64/65 的占位模式：官方下载链接 URL 在 preamble 注释 + 注释化的 `\documentclass` 切换指令 + compileHint warn ⚠️ + 回退到 article 的说明。`--list-templates` 现在列 9 个模板；`--compile-paper --latex-template llncs` / `--latex-template aaai` / `--latex-template ijcai` 立刻可用。94/94 paper-compiler 测试通过（原 81 + 新 13：3 个 preamble 字段断言 + 3 个 titleBlock 字段断言 + 3 个 compileHint 字段断言 + 3 个 hint 互相不同 + formatPaperLatex dispatch 3 个）。纯增量，不动 article / acmart / ieeeconf / iclr2026 / neurips / acl 行为。详见 `## 🤖 多智能体科研自动化` → `### 5.1 上手路径`。
- **2026-09-10** 🔄 `--diff-syntheses` synthesis diff（iter #66）：同一 session 的 2 份 `synthesis_*.md` 横向对比 — meta delta（rounds / deliverables / unique_papers / model / title / generated_at 时间戳）+ body delta（H1/H2/H3 主题 set diff:added / removed / shared + arXiv id refs set diff + 中英混合字数变化）+ Jaccard similarity（topics 0.7 + refs 0.3 加权，0-1）。新模块 `astro-src/lib/agents/synthesis-diff.{mjs,ts}`（与 export-bundle / paper-compiler / synthesis-pdf 同 Phase A shim 双 surface 共享模式）暴露 6 个纯函数：`stripFrontmatter` 浅解析 YAML frontmatter（scalar / array / boolean / numeric / CRLF），`extractSynthesisTopics` 抽 H1/H2/H3 主题列表（去 # 后缀 + 前导空格 + 保留中文），`extractSynthesisRefIds` regex `/(?:arxiv\.org\/abs\/|\b)(\d{4}\.\d{4,5})\b/g` 抽 arXiv canonical id + 去重 + 稳定排序，`countWords` 中文字符每个算 1 字 + 英文连字符 token 算 1 词，`diffSyntheses` 核心纯函数 + `formatSynthesisDiffText` CLI stdout 渲染。CLI 新 flag `--diff-syntheses <idxA> <idxB>`（pos 两个数字）+ `--json` 输出机器可读 JSON；找不到两份 synthesis 时 exit 2 + 清晰错误。44 个新单测覆盖 stripFrontmatter 5 + extractTopics 5 + extractRefs 6 + countWords 6 + diffSyntheses 15（含 meta delta 5 种 / body delta 4 种 / similarity 3 种 / 字节稳定 / 不变更输入 / null 边界 / includeBody:false）+ formatText 6 + 边界 + CRLF。新关闭 `docs/agents-workflow.md §6` "synthesis 之间没法对比" 候选。详见 `## 🤖 多智能体科研自动化` → `### 5.1 上手路径`。
- **2026-09-09** 🧬 `lib/agents/export-bundle.ts` typed 镜像（iter #60）：兑现 iter #59 末尾的承诺 — 给 iter #59 抽出的 `lib/agents/export-bundle.mjs` 配套 `.ts` 镜像，遵守项目 Phase A shim 策略（types.mjs 是 types.ts 的 runtime 镜像惯例）。`.ts` 文件集中定义 6 个 type contract（SessionMeta / RoundRecordSummary / SynthesisItem / ExportBundleStats / ExportBundle / ExportBundleInput）+ 通过 `export { ... } from './export-bundle.mjs'` re-export 运行时函数（不重复实现，避免 drift）。让 `orchestrator.ts` 等 TS caller 可以 typed import，新增 7/7 测试覆盖类型存在 + re-export plumbing + shape 兼容性（与 .mjs 实际输出结构一致）。详见 `## 🤖 多智能体科研自动化`。
- **2026-09-10** 📑 `--export-pdf` + `/agents/<sid>/pdf/` 浏览器合成 PDF 页（iter #63）：session 的 `synthesis/*.md` 全部合成 → 1 份**自包含打印就绪 HTML**（内嵌 CSS + `@page` + 页码 + 字体 fallback,3 种 CSS 变体 academic / compact / presentation）。用户在浏览器打开后 `Cmd/Ctrl+P` → "Save as PDF" 即可,**不依赖** pandoc / wkhtmltopdf 等系统 PDF 工具。新模块 `astro-src/lib/agents/synthesis-pdf.{mjs,ts}`（与 export-bundle / paper-compiler 同 Phase A shim 双 surface 共享策略）暴露 4 个纯函数 + 8 个 type contract：`buildPdfBundle` 把 meta + 所有 synthesis 装配成 PDFBundle(自动按 idx 排序),`formatPdfHtml` 输出 1 份自包含 HTML 含 cover + TOC + 每份 synthesis `<article>` + colophon,`buildPdfFileName(sid, idx)` 零填充 idx 生成 `<sid>-synthesis[-NNN].html`,`stripFrontmatter` 浅解析 YAML。CLI 新 flag `--export-pdf [PATH]` + `--pdf-style academic|compact|presentation` + `--json` 导出 PDFBundle;接受路径或目录,默认写 `archive/<sid>/synthesis.html`。同步上线 `/agents/<sid>/pdf/` 浏览器页(SSR 列所有有 meta 的 session + 数 archive/synthesis/ 数量 + 客户端 import 同一份 synthesis-pdf.mjs + 4 UI 控件:CSS variant 切换实时预览 + Download .html + Copy + Refresh + Back)。localStorage 限制同 iter #62:浏览器读不到 filesystem,从 round JSON 合成"伪 synthesis"(每 round 一份带 proposal 摘要)喂给 bundle;完整 archive/synthesis/ 内容仍需 CLI 端 `--export-pdf`。新增 50 个 lib 测试 + 24 个 UI 测试,覆盖 stripFrontmatter(标量/数组/布尔/CRLF)、buildPdfBundle(空 / 排序 / stats / fallback)、buildPdfFileName(padding / 安全字符)、formatPdfHtml(@page / 字节稳定 / 3 CSS 变体 / XSS 防护 / 链接 href 转义 / 边界)、md→html 子集(标题 / 列表 / 引用 / 链接 / 粗体 / 斜体 / 段落)。同步更新 docs/agents-workflow.md（关闭 §6 "synthesis 没法直接出 PDF" + 候选扩到 4 个 + 加 iter #63 入口示例）。详见 `## 🤖 多智能体科研自动化` → `### 5.1 上手路径` 与 `### 5.4 浏览器入口`。
- **2026-09-10** 📐 `--latex-template` + `PAPER_LATEX_TEMPLATES` 4 个 LaTeX 模板（iter #64）：`--compile-paper` 之前只支持 article class;iter #64 新增 `acmart`（ACM sigconf 单栏会议,自带 inputenc 不冲突）+ `ieeeconf`（IEEEtran conference proceedings,含 IEEEauthorblockN/A）+ `iclr2026`（占位,提示用户下载 iclr_conference.sty）。`PAPER_LATEX_TEMPLATES` 注册表集中管理每个模板的 `preamble`（string[]）+ `titleBlock`（fn,接收 title 返回 LaTeX 命令数组）+ `bibStyle`（统一 `thebibliography` 单 pass pdflatex）+ `compileHint`（编译提示字符串）。`formatPaperLatex` 重构:按 `opts.documentclass` 路由到模板,未识别 class 回退到 article(内容相同,只 preamble 不同)。CLI 新 flag `--latex-template T` / `--documentclass T` + `--list-templates`（列出全部模板 + 编译提示直接打印到 stdout）;辅助函数 `getLatexCompileHint(name)` + `listLatexTemplates()`。模板输出含 `% Template: <name>` + `% Compile: <hint>` header,便于用户定位模板与编译方式。73/73 paper-compiler 测试 + 242/242 跨 iter 测试通过(包括新模板的 preamble 差异 / title block 字段 / 字节稳定 / 未知 class fallback / acmart 不重加载 inputenc / ieeeconf 用 IEEEauthorblockN / iclr2026 提示下载 sty)。详见 `## 🤖 多智能体科研自动化` → `### 5.1 上手路径`。
- **2026-09-10** 📐 NeurIPS / ACL 模板加到 `PAPER_LATEX_TEMPLATES`（iter #65）：iter #64 注册表再加 2 个会议模板。`neurips`（NeurIPS 2024 + `preprint` 选项,占位提示下载 `neurips_2024.sty`）+ `acl`（ACL + `acl_natbib` + `times`,占位提示下载 `acl.sty` + `acl_natbib.sty`）。每个模板都有官方下载链接 (`media.neurips.cc` / `acl-org.github.io`) + 注释化的"取消下一行注释切到真 sty"指示 + 缺失回退到 article 的提示。`--list-templates` 现在列 6 个模板;`--compile-paper --latex-template neurips` / `--latex-template acl` 立刻可用。81/81 paper-compiler 测试(原 73 + 新 8:6 个 preamble 唯一性 + neurips / acl 的 preamble / title / compileHint 字段断言)+ 跨 iter 全套绿。不动 article / acmart / ieeeconf / iclr2026 行为,纯增量。详见 `## 🤖 多智能体科研自动化` → `### 5.1 上手路径`。
- **2026-09-09** 📑 `--compile-paper` session 碎片 → 论文装配器（iter #61）：关闭 `docs/agents-workflow.md §6` 第 1 号差距 "Modifier 不写 LaTeX" — 3 智能体循环往 `archive/<sid>/{drafts,reviews,experiments,paper_additions,rebuttals}/` 撒下的碎片 markdown 终于能装配成**一篇可编译的 LaTeX + markdown + .bib**。新模块 `astro-src/lib/agents/paper-compiler.{mjs,ts}`（与 export-bundle 同 Phase A shim 双 surface 共享策略）暴露 5 个纯函数 + 9 个 type contract：`buildPaperDraft` 路由 5 种 deliverable kind 到 5 个标准论文章节（Introduction / Related Work / Method / Results / Limitations），`formatPaperLatex` 输出可编译 LaTeX（pdflatex/xelatex 一次过）+ thebibliography，`formatPaperMarkdown` 输出 markdown 版，`collectBibliography` 从 proposal evidence.paperIds 自动去重抽 arXiv id 生成 BibTeX 引用表，`escapeLatex` 修复了 `\` → `\textbackslash{}` 转义后花括号被二次转义的经典 LaTeX bug + `\item` 方括号被吞的隐藏 LaTeX 坑。CLI 新 flag `--compile-paper [DIR]` + `--paper-format latex|markdown|both` + `--json` 导出 PaperDraft 结构化对象；6 行 demo 让 1 轮 session 在 10 秒内产出 `archive/<sid>/paper/{paper.md,paper.tex,refs.bib}`。同步修复 `tests/test_agents_mirror.mjs` 误报（re-export 形式 `export { a, b } from './x.mjs'` 之前会被 detector 判为 "MJS-only drift"，新增 `reExports` 桶 + `tsSideNames` 分类器按 .mjs 实际声明方式归类）。新增 44 个测试覆盖 5 类功能 + LaTeX 转义 + 字节稳定 + 空输入 + 防 LaTeX 坑。同步更新 docs/agents-workflow.md（关闭 §6 第 1 号 + 候选扩到 6 个）。详见 `## 🤖 多智能体科研自动化` → `### 5.1 上手路径`。
- **2026-09-09** 🌐 `/agents/<sid>/compile/` 浏览器装配页（iter #62）：iter #61 `--compile-paper` 的浏览器版本 —— 与 export.astro (iter #58) 同模式（SSR 列所有有 meta 的 session,客户端 import 同一份 paper-compiler.mjs 保证与 CLI 字节级一致）。**5 个 UI 控件**:format 切换 (LaTeX / Markdown / BibTeX 实时预览) + document class 切换 (article / acmart / ieeeconf / iclr2026) + Download (一次下载 3 个文件:paper.tex + paper.md + refs.bib) + Copy (剪贴板) + Refresh (重新读 localStorage)。**已知 iter #62 限制**:浏览器读不到文件系统,所以 localStorage round JSON 里的 proposal 元数据被合成成"伪 deliverable"(proposal.rationale + paperIds + risk 拼成 markdown),显示章节结构 + 引文 + 统计;**完整 deliverable 正文仍需 CLI 端** `node agents-run.mjs --compile-paper`。新增 25 个 UI 测试覆盖页面结构 + 客户端脚本(导入 paper-compiler、localStorage 合成、format/class 切换、3 文件下载、clipboard、boot)+ session dashboard 链接。详见 `## 🤖 多智能体科研自动化` → `### 5.4 浏览器入口`。
- **2026-09-09** 📦 Export bundle 抽 `lib/agents/export-bundle.mjs` 双 surface 共享（iter #59）：CLI (--export-md, iter #57) + 浏览器 (/agents/<sid>/export/, iter #58) 之前各有 ~150 LOC 的 mirror 实现，drift 风险高；iter #59 把 2 个纯函数 `buildExportBundle` + `formatExportMarkdown` 抽到 `astro-src/lib/agents/export-bundle.mjs` 单一真相源（与项目 Phase A shim 策略一致 .mjs 是 .ts 的 runtime 镜像），CLI 通过顶部 `import + re-export` 保持向后兼容（已有 14 个测试不动），浏览器页通过 `<script>` 的 ESM `import` 直接拿。新增 `tests/test_agents_export_bundle_lib.mjs` 13 个测试（含 CLI+browser+lib 字节级一致性断言）；同步更新 iter #57/58 测试 footer 标记为 iter #59；总测试 908 / pass 899 / 9 pre-existing fail 不变。详见 `## 🤖 多智能体科研自动化`。
- **2026-09-09** 🌐 `/agents/<sid>/export/` 浏览器导出页面（iter #58）：新增静态页 — SSR 列所有有 meta.json 的 session,客户端脚本 `buildExportBundle` + `formatExportMarkdown`（与 CLI iter #57 同结构同字段,可 cross-check）+ 3 个按钮（Download .md / Copy to clipboard / Preview）；优先读 localStorage(浏览器跑过的 round),fallback 到 SSR 提示用户用 CLI；session dashboard 加 "📦 导出" section 链接到 export 页 + CLI 等价命令提示。CSS 加 `.agents-export-section` + `.agents-help` + `.agents-help-warn` 3 个 class。18/18 测试通过(6 页面结构 + 8 客户端脚本 + 3 dashboard 链接 + 2 CSS)。详见 `## 🤖 多智能体科研自动化` → `### 浏览器入口`。
- **2026-09-09** 📦 单 session `--export-md` 一键打包（iter #57）：CLI 新增 `--export-md [PATH]`，把整个 session 打包成 1 份自包含 markdown（meta + 每 round 的 proposals/critiques/gate/modifier + 所有 synthesis + 最新 digest）；`PATH` 省略时默认写 `archive/<sid>/export.md`，传参可写任意路径；`--export-md --json` 输出 JSON 不写文件；用户可分享 / 归档 / 二次处理整个 session，不用分发整个 archive/ 文件夹。3 个 export 函数 `buildExportBundle` / `formatExportMarkdown` / `loadExportBundle` 全 export 便于测试，14/14 测试通过（4 纯函数 + 1 IO 错误处理 + 3 CLI plumbing + 3 e2e smoke 含 quickstart→export、JSON mode、自定义路径）。详见 `## 🤖 多智能体科研自动化` → `### CLI Runner`。
- **2026-09-09** 🔎 Designer 加 `--search-arxiv` 真实工具调用（iter #56）：CLI 新增 `--search-arxiv "<QUERY>"`，在 round 跑之前调 arXiv API（export.arxiv.org）拉真实论文作为 candidates，**关闭与 Sakana/STORM/OpenAI Deep Research "tool use" 的最大短板**——之前 Designer 只能用 `archive/<sid>/recommend/` 里的旧论文；现在用户从 0 个候选论文起步也能启动 session；纯 IO 函数 `searchArxivApi` + `parseArxivEntry` + `canonicalArxivId` 全 export 便于测试，21/21 单测 + e2e 通过（包含 sandbox 网络断开时优雅回退到 0 candidates 继续 round）。详细对比见 [`docs/agents-workflow.md`](docs/agents-workflow.md) §6 "已知限制"。
...
# 真跑时用 --preset aggressive 让 stub LLM score=5 触发 candidate 决策,
# 演示 Modifier 真写 .md deliverables(否则 stub 走 sketch 不写盘)
node astro-src/scripts/agents-run.mjs --new-session "deliv demo" --rounds 1 --dry-run --preset aggressive
ls archive/<sid>/drafts/   # create_draft 类型
ls archive/<sid>/reviews/  # literature_review 类型
...
**Modifier 真写 deliverables(iter #32 + #35 + #38,5 type 全覆盖)**:当 proposal 类型 + gate 决策是 `promoted` / `candidate` 时,Modifier 真把 markdown 写到:
- `create_draft` → `archive/<sid>/drafts/draft_r<NNN>_<idx>.md`(frontmatter + 动机 / 证据 / 风险 / 草稿正文)
- `literature_review` → `archive/<sid>/reviews/review_r<NNN>_<idx>.md`(摘要 / 引用论文 / 主题分类 / 关键 Gap / 推荐阅读顺序)
- `experiment_plan` → `archive/<sid>/experiments/exp_r<NNN>_<idx>.md`(假设 / 方法 / 数据集 / 评估指标 / 算力 / 相关论文 / 风险)
- `add_paper` → `archive/<sid>/paper_additions/paper_r<NNN>_<idx>.md`(📄 title + 加入理由 / 候选论文 IDs / 风险 / 下一步)
- `rebuttal` → `archive/<sid>/rebuttals/rebuttal_r<NNN>_<idx>.md`(✉️ title + 摘要 / 目标论文 / Reviewer Comments / Responses / 论文修改 / 风险)

dry-run 时正文带 ⚠️ DRY-RUN 占位符;LLM 真跑可通过 `ctx.body` 注入真实内容(method / dataset / metrics / compute / hypothesis / reviewer_comments / responses / changes)。幂等:同 (round, idx) 二次调用不覆盖。`formatDraftMarkdown` / `formatReviewMarkdown` / `formatExperimentMarkdown` / `formatAddPaperMarkdown` / `formatRebuttalMarkdown` / `writeDeliverable` 是 export 纯函数 / 异步 IO,`tests/test_agents_modifier_deliverables.mjs`(22 case)+ `test_agents_modifier_experiment.mjs`(16 case)+ `test_agents_modifier_full_types.mjs`(18 case)全过。**Modifier 现在覆盖 100% ProposalType**,5/5 全部走 writeDeliverable。

**两轮 diff(iter #33 + #37 title-fuzzy)**:`--diff --session SID <A> <B>`(两个 round 数字作 positional arg)比较同 session 两 round 的 proposals added / removed / changed + score delta + decision transition(gate 桶变化);changed 按 |scoreDelta| 倒序展示波动最大的 proposal;`--json` 输出机器可读格式;找不到 round 时 exit 2 + 清晰错误信息。
- iter #33 基础:按 `proposal.id` 匹配,适合 LLM 保持 id 稳定的情况
- iter #37 fuzzy fallback:对 id 没配上的 proposals 跑 title-Jaccard 匹配(英文按词、CJK 按字拆 token,Jaccard >= 0.5 算同一个);让 stub LLM(每轮 random id)和真实 LLM 微调 id 场景下 diff 都有意义。`diffRounds` 输出 `stats.idMatched` + `stats.fuzzyMatched`;`formatDiffText` 在 Stats 行加 `[matched: N by-id, M fuzzy]`,Changed 行 fuzzy 配的会标 `[fuzzy=0.85, p1≡q2]`。`diffRounds` / `diffRounds` / `formatDiffText` / `loadDiff` 是 export 纯函数 / 异步 IO;`tests/test_agents_diff_cli.mjs` 14 个 case + `tests/test_agents_diff_fuzzy_cli.mjs` 26 个 case 全过。`normalizeTitle` / `titleJaccard` / `findTitleFuzzyMatches` 都是独立 export 的纯函数,便于复用。
