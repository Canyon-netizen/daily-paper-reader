...
**Modifier 真写 deliverables(iter #32 + #35)**:当 proposal 类型是 `create_draft` / `literature_review` / `experiment_plan` 且 gate 决策是 `promoted` / `candidate` 时,Modifier 真把 markdown 写到:
...
dry-run 时正文带 ⚠️ DRY-RUN 占位符;LLM 真跑可通过 `ctx.body` 注入真实内容(method / dataset / metrics / compute / hypothesis)。幂等:同 (round, idx) 二次调用不覆盖。`formatDraftMarkdown` / `formatReviewMarkdown` / `formatExperimentMarkdown` / `writeDeliverable` 是 export 纯函数 / 异步 IO,`tests/test_agents_modifier_deliverables.mjs`(22 case)+ `tests/test_agents_modifier_experiment.mjs`(16 case)全过。`add_paper` / `rebuttal` 暂时仍走 `archive_round_summary`(后续可加)。
