#!/usr/bin/env python
"""Init script for bioRxiv — fetches papers and syncs to Supabase.
...
# Constants the parent module used to expose; kept here so direct imports
# from this script keep working even after the factory extraction.
...
def main() -> None:
    ...
    raw_path = resolve_raw_path(
        ...
        date_str=date_str,
        default_filename=f"biorxiv_papers_{date_str}.json",
    )

    if not args.skip_fetch:
        fetch_cmd = [
            python_executable(),
            os.path.join(os.path.dirname(__file__), "fetchers", "fetch_biorxiv_family.py"),
            "--source", "biorxiv",
            "--days", str(max(int(args.days or 1), 1)),
            "--chunk-days", str(max(int(args.chunk_days or 1), 1)),
            "--output", raw_path,
        ]
        if args.ignore_seen:
    ...
    run_step("Step 2 - sync bioRxiv to Supabase", build_sync_cmd(
        backend_key="biorxiv",
        date_str=date_str,
        raw_path=raw_path,
        args=args,
