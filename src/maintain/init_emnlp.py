#!/usr/bin/env python
"""Init script for EMNLP Anthology — fetches papers and syncs to Supabase.
...
def main() -> None:
    ...
    raw_filename = f"emnlp-anthology-{year_start}-{year_end}.json"
    ...
    if not args.skip_fetch:
        fetch_cmd = [
            python_executable(),
            os.path.join(os.path.dirname(__file__), "fetchers", "fetch_acl_anthology.py"),
            "--conference", "EMNLP",
    ...
    run_step("Step 2 - sync EMNLP papers to Supabase", build_sync_cmd(
        backend_key="emnlp",
        date_str=date_str,
        raw_path=raw_path,
        args=args,
