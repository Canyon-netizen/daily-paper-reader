...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _is_local_client(self) -> bool:
        """返回 True 表示请求来自 loopback。

        local debug server 的安全前提:只响应来自同一台机器的请求。
        如果 server 被显式绑 0.0.0.0(LAN 调试),我们仍要把敏感端点
        (secret 读写)锁在 loopback,避免 LAN 设备 dump 全部 LLM/Supabase key。
        """
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/local/secret":
            if not self._is_local_client():
                return self._json({"ok": False, "error": "secret endpoint is loopback-only"}, status=403)
            # 只返回存在性 + 路径,不返回 payload 本身,避免 payload 在
            # 浏览器 / 抓包 / 任何中间层明文泄露。
            return self._json({
                "ok": True,
