...
# LONG_RANGE_DAYS_THRESHOLD: ≥此天数时 daily pipeline 生成的目录使用区间标签
#   (YYYYMMDD-YYYYMMDD,见 src/main.py:179 build_run_date_token),从而被 Astro 路由
#   [period]/[arxiv].astro 命中。详见 docs/path-spec.md §6。
MAIN_DEFAULT_DAYS = 9
# MAIN_DEFAULT_DAYS: config.yaml.arxiv_paper_setting.days_window 的默认值;
#   < LONG_RANGE_DAYS_THRESHOLD,走单日标签(YYYYMMDD),生成 docs/YYYYMMDD/README.md。
#   改这个值会同时改变 docs/ 下用户首次跑出来的目录结构,要同步提示 README。
...
def load_gist_env() -> None:
    """
    从 GITHUB_ENV 文件加载 Gist loader 注入的变量到当前进程 os.environ。
    子进程用 subprocess.run 启动时会继承 os.environ,这样 LLM_MODEL
    等 key 就能传到子进程里。
    """
    gist_env_path = os.environ.get("GITHUB_ENV")
    if not gist_env_path or not os.path.exists(gist_env_path):
        return
    try:
        with open(gist_env_path, encoding="utf-8") as fp:
            for raw in fp:
                line = raw.strip()
...
def apply_topics_from_gist_env() -> None:
    """
    当 paper-analyzer 同步 Gist 时携带 topics 字段,Gist loader step
    写 TOPICS_JSON env(由用户在纸-analyzer→"同步到 Gist" 触发)。
    我们这里:
      1. 尝试从 env 读取 TOPICS_JSON,如果存在(env loader 已写)
      2. 也尝试从 /tmp/dpr-gist.json 直接读(Gist loader 总下载这文件,
         跳过 env 编码问题)
      3. 替换 config.yaml 中 subscriptions.intent_profiles
      4. 写盘(让后续 subprocess 调起的 step 都看到)

    写盘是必要的:subprocess 启动独立 Python 进程,不能共享内存。
    workflow 跑完盘上 config.yaml 不影响仓库(没 git push,只是临时改写)。
    """
    # 路径 1:env(标准路径)
    raw = os.environ.get("TOPICS_JSON")

    # 路径 2:Gist loader 已写到 /tmp/dpr-gist.json(env 编码有问题时 fallback)
    if not raw:
        gist_json_path = "/tmp/dpr-gist.json"
        if os.path.exists(gist_json_path):
            try:
                data = json.load(open(gist_json_path, encoding="utf-8"))
    ...
    try:
        ...
        print(f"[WARN] TOPICS_JSON parse failed: {e}", flush=True)
        return
    if not isinstance(topics, list) or not topics:
        return
    if not os.path.exists(CONFIG_FILE):
        print(
            f"[WARN] config.yaml not found, cannot apply TOPICS_JSON "
            f"({len(topics)} profiles ignored)",
            flush=True,
        )
        return
    try:
        ...
        print(f"[WARN] config.yaml read failed: {e}", flush=True)
        return
    subs = data.setdefault("subscriptions", {})
    old_count = len(subs.get("intent_profiles") or [])
    subs["intent_profiles"] = topics
    try:
        save_config(data, CONFIG_FILE)
        print(
            f"[INFO] applied {len(topics)} intent profiles from Gist "
            f"(overrode {old_count} from config.yaml)",
            flush=True,
        )
    except Exception as e:
        print(f"[WARN] config.yaml write failed: {e}", flush=True)
...
def main() -> None:
    ...
    # 能继承 LLM_MODEL / LLM_API_KEY 等 secret。
    load_gist_env()
    # 用户在 paper-analyzer → "同步到 Gist" 时如果带了 topics 字段,
    # Gist loader 会写 TOPICS_JSON env,我们把它应用到 config.yaml,
    # 这样下游 step 用新的主题配置。
    apply_topics_from_gist_env()
