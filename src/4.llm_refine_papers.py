...
def process_file(
    ...
    # 与模块顶部 DEFAULT_FILTER_MODEL 同样的 fallback 链:LLM_MODEL → MINIMAX_FILTER_MODEL → BLT_FILTER_MODEL
    # 这样仓库 Settings 只配 MINIMAX_FILTER_MODEL (或 BLT_FILTER_MODEL) 也能跑通,
    # 不必严格用 LLM_MODEL。
    model_env = (
        os.getenv("LLM_MODEL")
        or os.getenv("MINIMAX_FILTER_MODEL")
        or os.getenv("BLT_FILTER_MODEL")
    )
    if not model_env:
        raise RuntimeError(
            "缺少 LLM_MODEL 环境变量，请设置 LLM_MODEL 或 MINIMAX_FILTER_MODEL"
            "（provider/model 格式，例如 'minimax/MiniMax-M2.7'）"
        )
