...
def user_config_path(default_path: str) -> str:
    """
    Return the path of the fork-user override file for the given
    base config path, honoring the `DPR_USER_CONFIG` env override.

    Convention:
        foo/config.yaml  →  foo/config.user.yaml
        foo/config.local.yaml  →  foo/config.user.local.yaml
        (DPR_USER_CONFIG=/abs/path.yml) → that absolute path

    Default cwd lookups: repo root for the canonical config.yaml,
    nothing else auto-discovered. Users who want non-default
    placement must set `DPR_USER_CONFIG`.
    """
    env_override = str(os.getenv("DPR_USER_CONFIG") or "").strip()
    if env_override:
        return env_override
    base_dir = os.path.dirname(default_path) or "."
    base_name = os.path.basename(default_path)
    if base_name.endswith(".yaml"):
        stem, ext = base_name[:-5], ".yaml"
    elif base_name.endswith(".yml"):
        stem, ext = base_name[:-4], ".yml"
    else:
        stem, ext = base_name, ".yaml"
    return os.path.join(base_dir, f"{stem}.user{ext}")
...
def _deep_merge(base: Dict[str, Any], override: Dict[str, Any]) -> Dict[str, Any]:
    """
    Recursive merge: `override` keys replace `base`, BUT for keys
    whose value is a dict in BOTH sides, we merge inside the dict
    instead of replacing (so fork users can override e.g. only
    `subscriptions.intent_profiles[3].paper_sources` without
    losing the rest of that profile / the rest of subscriptions).
    """
    ...
    for key, value in (override or {}).items():
        ...
        if isinstance(existing, dict) and isinstance(value, dict):
            ...
            # Lists are replaced wholesale — deep-merging lists is
            # ambiguous (positional vs identity). Override semantics
            # are clearer for the fork-user case.
...
def _merge_dicts(base: Dict[str, Any], override: Dict[str, Any]) -> Dict[str, Any]:
    """Flat (non-recursive) merge kept for legacy callers."""
    out = copy.deepcopy(base if isinstance(base, dict) else {})
    for key, value in (override or {}).items():
        out[key] = value
    return out
...
def load_config_with_source_migration(path: str, *, write_back: bool = True) -> Dict[str, Any]:
    if yaml is None:
        return {}
    base = _load_yaml_dict(path)
    if not base:
        # Nothing in the base file — still consult the user overlay
        # so a fork user with a config that has been pruned to
        # empty can still seed everything from config.user.yaml.
        base = {}

    user_path = user_config_path(path)
    if user_path == path:
        overlay: Dict[str, Any] = {}
    else:
        overlay = _load_yaml_dict(user_path)

    if overlay:
        # Apply the deep merge before source migration so that
        # migration sees the final shape (and `write_back=True`
        # round-trips the merged dict).
        merged: Dict[str, Any] = _deep_merge(base, overlay)
    else:
        merged = base

    if not merged:
        # Don't touch disk if there's nothing to normalize.
        return {}

    changed, _notes = migrate_source_config_inplace(merged)
    if changed and write_back and user_path != path:
        # Persist migration fixes only to the canonical config,
        # never to the user overlay. This means a fork user's
        # config.user.yaml is read-only from our side.
