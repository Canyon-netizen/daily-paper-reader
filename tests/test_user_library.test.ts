...
describe('user-library/gist', () => {
  let gist: typeof import('../astro-src/lib/user-library/gist');

  beforeEach(async () => {
    const gId = require.resolve('../astro-src/lib/user-library/gist');
    delete require.cache[gId];
    gist = await import('../astro-src/lib/user-library/gist');
  });
  ...
  it('serialize / deserialize roundtrip', () => {
    const doc = {
      ...
      papers: {
        '2607.00001': { updatedAt: 100, starred: true },
        '2607.00002': { updatedAt: 200, note: 'hello world' },
      },
    };
    const text = gist.serializeUserLibrary(doc);
    const parsed = gist.deserializeUserLibrary(text);
    expect(parsed).toEqual(doc);
  });

  it('deserialize returns null on corrupt JSON', () => {
    expect(gist.deserializeUserLibrary('not json')).toBe(null);
    expect(gist.deserializeUserLibrary('null')).toBe(null);
    expect(gist.deserializeUserLibrary('{"schemaVersion":99}')).toBe(null);
  });

  it('merge: empty local + remote → remote wins', () => {
    const { merged, counters } = gist.mergeUserLibrary(
      { schemaVersion: 1, papers: {} },
      { schemaVersion: 1, papers: { '2607.00001': { updatedAt: 1, starred: true } } },
    );
    expect(Object.keys(merged.papers)).toEqual(['2607.00001']);
    expect(counters.writtenPapers).toBe(1);
    expect(counters.conflicts).toBe(0);
  });

  it('merge: same key, both have only star → newer wins', () => {
    const { merged, counters } = gist.mergeUserLibrary(
      { schemaVersion: 1, papers: { '2607.00001': { updatedAt: 100, starred: true } } },
      { schemaVersion: 1, papers: { '2607.00001': { updatedAt: 200, starred: false } } },
    );
    expect(merged.papers['2607.00001'].starred).toBe(false);
    expect(counters.conflicts).toBe(0);
  });

  it('merge: both have notes, different content → conflict marker', () => {
    const { merged, counters } = gist.mergeUserLibrary(
      { schemaVersion: 1, papers: { '2607.00001': { updatedAt: 100, note: 'local note' } } },
      { schemaVersion: 1, papers: { '2607.00001': { updatedAt: 200, note: 'remote note' } } },
    );
    expect(counters.conflicts).toBe(1);
    expect(merged.papers['2607.00001'].note).toContain('local note');
    expect(merged.papers['2607.00001'].note).toContain('<<<<<<< remote');
    expect(merged.papers['2607.00001'].note).toContain('remote note');
  });

  it('merge: note text identical but timestamps differ → no conflict', () => {
    const { merged, counters } = gist.mergeUserLibrary(
      { schemaVersion: 1, papers: { '2607.00001': { updatedAt: 100, note: 'same' } } },
      { schemaVersion: 1, papers: { '2607.00001': { updatedAt: 200, note: 'same' } } },
    );
    expect(counters.conflicts).toBe(0);
    expect(merged.papers['2607.00001'].updatedAt).toBe(200);
  });

  it('merge: canonical key normalizes v1/v2', () => {
    const { merged } = gist.mergeUserLibrary(
      { schemaVersion: 1, papers: { '2607.00001v1': { updatedAt: 100, starred: true } } },
      { schemaVersion: 1, papers: { '2607.00001v2': { updatedAt: 200, starred: true } } },
    );
    expect(Object.keys(merged.papers)).toEqual(['2607.00001']);
  });

  it('merge: dedupes between different canonical entries that map to same key', () => {
    const { merged } = gist.mergeUserLibrary(
      { schemaVersion: 1, papers: {
        '2607.00001v1': { updatedAt: 100, starred: true },
        '2607.00001': { updatedAt: 50, starred: false },
      } },
      { schemaVersion: 1, papers: {} },
    );
    expect(Object.keys(merged.papers)).toEqual(['2607.00001']);
    expect(merged.papers['2607.00001'].starred).toBe(true);
  });
});
