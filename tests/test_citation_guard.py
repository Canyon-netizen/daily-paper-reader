...
class TestS2RateLimitBackoff(unittest.TestCase):
    ...
    def test_429_then_success(self):
        ...
        with patch("citation_guard.urllib.request.urlopen", side_effect=side_effects):
            hits, ok = search_semantic_scholar("Some Paper Title", 2023, sleep_fn=sleeps.append)
    ...
    def test_429_exhausted_returns_empty(self):
        ...
        sleeps: list[float] = []
        with patch("citation_guard.urllib.request.urlopen", side_effect=side_effects):
            hits, ok = search_semantic_scholar("X", None, retry_max=5, sleep_fn=sleeps.append)
        self.assertEqual(hits, [])
        # 4 次 sleep(最后一次 429 后不再 sleep)
        self.assertEqual(len(sleeps), 4)
        # 429 退避耗尽视为网络失败(network_ok=False)—— plan §4.3 验收 2
        self.assertFalse(ok)
...
# === plan §4.3 双轴 evidence 模型 ===

class TestNetworkFailureReturnsNotChecked(unittest.TestCase):
    """plan §4.3 验收 2:网络不可用时,远程两源全部失败,不应判定 fabricated。

    之前的 bug:search_semantic_scholar / search_openalex 在网络挂时返
    [],caller 把 [] 当作「远程确认无此文献」,标 fabricated。这是假阳性,
    论文被错误降级。本次 commit 把 search_* 改成返 (hits, network_ok),
    check_citation_existence 在两源都 network_ok=False 时返 'not_checked'。
    """

    def test_both_apis_unreachable_returns_not_checked(self):
        cite = {"title": "Some Imaginary Paper", "year": 2099}
        with patch("citation_guard.search_library", return_value=None), \
             patch("citation_guard.search_semantic_scholar", return_value=([], False)), \
             patch("citation_guard.search_openalex", return_value=([], False)):
            existence, match = check_citation_existence(cite, [])
        self.assertEqual(existence, "not_checked")
        self.assertEqual(match["source"], "none")
        self.assertIn("unreachable", match["reason"])

    def test_only_one_api_unreachable_still_fabricates_if_other_has_hits(self):
        """一源网络挂 + 另一源 API OK 但 0 hits → 仍然 fabricated。
        网络挂只是 network_ok=False,不代表「远程确认无此文献」。
        """
        cite = {"title": "Imaginary Paper", "year": 2099}
        with patch("citation_guard.search_library", return_value=None), \
             patch("citation_guard.search_semantic_scholar", return_value=([], False)), \
             patch("citation_guard.search_openalex", return_value=([], True)):
            existence, _ = check_citation_existence(cite, [])
        self.assertEqual(existence, "fabricated")

    def test_one_api_unreachable_other_finds_minor(self):
        """S2 网络挂,OpenAlex 找到 minor → 仍然 minor(只要有一源 OK 且有命中)。"""
        cite = {"title": "Attention Is All You Need", "year": 2017}
        with patch("citation_guard.search_library", return_value=None), \
             patch("citation_guard.search_semantic_scholar", return_value=([], False)), \
             patch("citation_guard.search_openalex", return_value=([
                 {"title": "Attention Is All You Need (minor)", "year": 2017}
             ], True)):
            existence, match = check_citation_existence(cite, [])
        self.assertEqual(existence, "minor")
        self.assertEqual(match["source"], "openalex")
...
class TestReviewPassedDualAxis(unittest.TestCase):
    """plan §4.3 验收 4:pass 逻辑不再把 minor 直接当 support。

    之前 review_passed 在没有支持性 check 时直接 True,即使有 minor / not_checked
    也算 pass。这次要求:
      - fabricated 出现 → False(始终)
      - 没跑 support check 时,要求 existence 全是 exact 才 pass
        (minor / not_checked 保守视为未通过)
    """

    def test_no_support_check_passes_only_with_all_exact(self):
        summary = {"exact": 3, "minor": 0, "fabricated": 0, "not_checked": 0,
                   "supported": 0, "partial": 0, "unsupported": 0}
        citations = [{"existence": "exact"} for _ in range(3)]
        self.assertTrue(review_passed(summary, citations))

    def test_no_support_check_with_minor_does_not_pass(self):
        """有 minor 但没跑 support → 不 pass(plan §4.3 验收 4)。"""
        summary = {"exact": 2, "minor": 1, "fabricated": 0, "not_checked": 0,
                   "supported": 0, "partial": 0, "unsupported": 0}
        citations = (
            [{"existence": "exact"}] * 2 + [{"existence": "minor"}]
        )
        self.assertFalse(review_passed(summary, citations))

    def test_no_support_check_with_not_checked_does_not_pass(self):
        """网络挂了 → 没法确认,不应 pass。"""
        summary = {"exact": 2, "minor": 0, "fabricated": 0, "not_checked": 1,
                   "supported": 0, "partial": 0, "unsupported": 0}
        citations = (
            [{"existence": "exact"}] * 2 + [{"existence": "not_checked"}]
        )
        self.assertFalse(review_passed(summary, citations))

    def test_fabricated_always_fails(self):
        """fabricated 是硬规则,任何 support check 都不救。"""
        summary = {"exact": 0, "minor": 0, "fabricated": 1, "not_checked": 0,
                   "supported": 100, "partial": 0, "unsupported": 0}
        citations = [{"existence": "fabricated"}]
        self.assertFalse(review_passed(summary, citations))

    def test_support_checked_with_not_checked_in_summary(self):
        """分母只算 checked 部分,not_checked 不算分母(plan §4.3 「分别计数」)。"""
        summary = {"exact": 1, "minor": 0, "fabricated": 0, "not_checked": 2,
                   "supported": 1, "partial": 0, "unsupported": 0}
        # 1 个 exact + 2 个 not_checked,supported/checked = 1/1 = 1.0 ≥ 0.6 → pass
        citations = (
            [{"existence": "exact"}] + [{"existence": "not_checked"}] * 2
        )
        self.assertTrue(review_passed(summary, citations))


class TestExtractClaimContext(unittest.TestCase):
    def test_finds_marker_and_extracts_context(self):
        md = "这是一段测试文字。这里提到 [1] 引用。后面还有更多内容。" * 5
        ctx = _extract_claim_context(md, "[1]")
        self.assertIn("[1]", ctx)
        self.assertIn("引用", ctx)

    def test_marker_not_found_returns_empty(self):
        md = "没有任何引用标记的文档"
        ctx = _extract_claim_context(md, "[999]")
        self.assertEqual(ctx, "")

    def test_truncates_at_boundaries(self):
        md = "[1] 开始后面有更多内容"
        ctx = _extract_claim_context(md, "[1]", window=5)
        # window=5 means 5 chars on each side, so we get "[1] 开"
        self.assertIn("[1]", ctx)


class TestCheckCitationSupport(unittest.TestCase):
    def test_no_fulltext_returns_not_checked(self):
        citation = {"marker": "[1]", "title": "Some Paper", "year": 2024}
        result = check_citation_support(citation, "claim context", None)
        self.assertEqual(result, "not_checked")

    def test_supported_via_mock_call(self):
        citation = {"marker": "[1]", "title": "Some Paper", "year": 2024}
        mock_call = lambda prompt: {"support": "supported", "reason": "论文明确支持"}
        result = check_citation_support(
            citation, "正文引用了该论文", "paper abstract", call=mock_call
        )
        self.assertEqual(result, "supported")

    def test_partial_via_mock_call(self):
        citation = {"marker": "[1]", "title": "Some Paper", "year": 2024}
        mock_call = lambda prompt: {"support": "partial", "reason": "部分支持"}
        result = check_citation_support(
            citation, "正文引用了该论文", "paper abstract", call=mock_call
        )
        self.assertEqual(result, "partial")

    def test_unsupported_via_mock_call(self):
        citation = {"marker": "[1]", "title": "Some Paper", "year": 2024}
        mock_call = lambda prompt: {"support": "unsupported", "reason": "矛盾"}
        result = check_citation_support(
            citation, "正文引用了该论文", "paper abstract", call=mock_call
        )
        self.assertEqual(result, "unsupported")

    def test_llm_failure_returns_not_checked(self):
        citation = {"marker": "[1]", "title": "Some Paper", "year": 2024}
        mock_call = lambda prompt: (_ for _ in ()).throw(Exception("LLM error"))
        result = check_citation_support(
            citation, "正文引用了该论文", "paper abstract", call=mock_call
        )
        self.assertEqual(result, "not_checked")

    def test_invalid_support_value_returns_not_checked(self):
        citation = {"marker": "[1]", "title": "Some Paper", "year": 2024}
        mock_call = lambda prompt: {"support": "unknown_value", "reason": "bad"}
        result = check_citation_support(
            citation, "正文引用了该论文", "paper abstract", call=mock_call
        )
        self.assertEqual(result, "not_checked")
...
class TestRunGuardSupportCheck(unittest.TestCase):
    def test_run_support_check_false_by_default(self):
        """默认不跑 support check (backward compat)"""
        with tempfile.TemporaryDirectory() as td:
            tmp = Path(td)
            md = tmp / "2510.00004v1-test.md"
            md.write_text(
                "正文 [1]。\n\n## 参考文献\n\n"
                "[1] Real Paper, 2024. Real Paper Title\n",
                encoding="utf-8",
            ...
            # All citations stay not_checked for support
            for c in result["citations"]:
                self.assertEqual(c["support"], "not_checked")

    def test_run_support_check_true_calls_support(self):
        """run_support_check=True 时跑 support check"""
        with tempfile.TemporaryDirectory() as td:
            tmp = Path(td)
            md = tmp / "2510.00005v1-test.md"
            md.write_text(
                "正文 [1] 引用了该论文[1]。\n\n## 参考文献\n\n"
                "[1] Real Paper, 2024. Real Paper Title\n",
                encoding="utf-8",
            )
            mock_call = lambda prompt: {"support": "supported", "reason": "明确支持"}
            with patch("citation_guard.search_library", return_value={
                "source": "library", "paper_id": "x", "title": "Real Paper Title",
                "year": 2024, "similarity": 1.0, "year_tolerance": 0
            }):
                result = run_guard(md, {
                    ...
                    "paper_fulltext": "这是论文全文摘要内容",
                    "support_call": mock_call,
                })
            # Support check ran
            self.assertEqual(result["summary"]["supported"], 1)
            self.assertEqual(result["citations"][0]["support"], "supported")

    def test_run_support_check_skips_fabricated(self):
        """support check 只对 existence in {exact, minor} 跑"""
        with tempfile.TemporaryDirectory() as td:
            tmp = Path(td)
            md = tmp / "2510.00006v1-test.md"
            md.write_text(
                "正文 [1] 和 [2]。\n\n## 参考文献\n\n"
                "[1] Real Paper, 2024. Real Paper Title\n"
                "[2] Fake Paper, 2099. Nonexistent\n",
                encoding="utf-8",
            )
            mock_call = lambda prompt: {"support": "supported", "reason": "支持"}

            # Make search_library return match only for "Real Paper"
            ...
            # First citation: exact + support checked
            self.assertEqual(result["citations"][0]["existence"], "exact")
            self.assertEqual(result["citations"][0]["support"], "supported")
            # Second citation: fabricated, should stay not_checked for support
            self.assertEqual(result["citations"][1]["existence"], "fabricated")
            self.assertEqual(result["citations"][1]["support"], "not_checked")
