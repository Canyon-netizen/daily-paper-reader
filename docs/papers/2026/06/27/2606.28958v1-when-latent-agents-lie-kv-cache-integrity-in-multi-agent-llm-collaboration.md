...
method_pros_cons: {"Full-KV Latent Collaboration": {"pros": ["在HiddenBench上EM/F1达0.338/0.486，优于文本协作", "拆证据聚合能力强，特异性影响10/21答案", "Qwen3-8B和HotPotQA全验证集均显示方向性提升"], "cons": ["EM提升统计显著性较弱", "每个样本约279MB KV缓存，传输开销大", "隐藏状态易受恶意特化代理操纵，完整性脆弱"]}, "Text Collaboration": {"pros": ["实现简单，可见消息易于审计和检查", "不传输敏感隐藏状态，攻击面更小", "作为基线提供公平的可比对照"], "cons": ["HiddenBench上EM/F1仅0.231/0.369，性能较低", "缺乏丰富的隐藏证据传递能力", "无法利用特化代理的中间推理状态"]}, "HMAC-SHA256 Manifest": {"pros": ["接受774/774诚实负载，拒绝295/295篡改负载", "绑定身份、会话、模型、张量元数据等多维信息", "直接在传输层保护完整性，抗在途篡改"], "cons": ["无法保护被攻陷的端点签署的恶意KV", "不涵盖语义层面的恶意特化代理", "依赖传输密钥未被泄露的假设"]}, "Visible Verifier Filtering": {"pros": ["实现简单，仅审计可见承诺文本", "可过滤部分语义错误承诺", "与文本协作协议无缝集成"], "cons": ["仅审计承诺不验证隐藏KV，遗漏关键失效模式", "对隐藏态腐败和缩放操纵完全无效", "无法检测规范匹配的适应性攻击"]}, "Magnitude Quarantine": {"pros": ["能捕获明显的非语义高范数腐败攻击", "计算开销低，易于部署", "对随机隐藏态损坏有基本防御"], "cons": ["范数匹配的自适应攻击可完全规避检测", "HiddenBench白盒攻击EM降至0.077未被检出", "100条HotPotQA中0/100恶意状态被拒绝"]}, "Learned Sanitizer": {"pros": ["可学习复杂的隐藏态异常模式", "在完整65条记录上经过校准", "提供数据驱动的异常检测方案"], "cons": ["在检测器感知攻击下不及随机预算丢弃", "面临适应性攻击时性能下降显著", "需要标注数据且泛化能力受限"]}, "Fail-Closed Rejection": {"pros": ["检测到传输失败后HiddenBench恢复至0.338/0.476", "HotPotQA降级至0.460/0.597保持可用性", "安全优先策略避免信任受损的隐藏态"], "cons": ["丢失潜在有用信息，性能略低于完整通过", "降级模式牺牲部分协作增益", "依赖上游检测机制的正确触发"]}}
method_comparison: "全文在传输完整性与异常检测之间形成鲜明对比：基于隐藏态检测的方案（可见验证、范数隔离、学习净化器）均被适应性攻击规避，而传输层HMAC-SHA256清单机制以774/774与295/295的完美识别率确立了可靠的安全边界。"
method_debate_generated_at: "2026-09-01T11:52:36.431326Z"
