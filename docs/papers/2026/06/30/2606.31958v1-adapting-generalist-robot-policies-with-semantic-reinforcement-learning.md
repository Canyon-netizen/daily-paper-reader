...
method_pros_cons: {"SARL (Semantic Action Reinforcement Learning)": {"pros": ["在语义提示空间优化，结构化探索效率高", "不到100个回合即可完成真实机器人适配", "利用预训练VLA先验，无需从零学习技能", "提示与实际行为对齐，长horizon任务表现突出"], "cons": ["依赖VLA预训练覆盖足够广泛的语言诱导技能", "需在线交互训练，实际部署成本较高", "语言提示空间的选择与任务语义耦合，设计复杂"]}, "Residual Policy Training": {"pros": ["保留VLA原策略，可叠加修正行为", "实现简单，兼容现有VLA架构"], "cons": ["仍需低层动作接近最优策略，难以解决长horizon任务", "面对预训练分布外的复杂任务能力有限"]}, "Denoising Process Steering (Flow/Diffusion)": {"pros": ["可细粒度调控生成过程，灵活性较高"], "cons": ["仅适用于扩散或流模型结构的VLA，通用性受限", "仍作用于机器人动作层，长horizon任务受限"]}, "Advantage Conditioning": {"pros": ["通过条件信号引导策略改善训练稳定性"], "cons": ["需修改VLA训练流程，实现门槛较高", "未改变动作优化空间，长horizon能力有限"]}, "Zero-shot VLA Deployment": {"pros": ["无需额外训练，直接利用VLA预训练能力", "部署简单，开箱即用"], "cons": ["复杂长horizon任务难以成功，缺乏任务分解能力", "无法超越VLA原始训练分布"]}, "VLM In-Context Learning (High-level Language Steering)": {"pros": ["利用VLM常识进行高层任务分解，无需训练", "灵活利用已有VLA技能"], "cons": ["提示未必对应VLA可执行的接地行为，易失败", "缺乏在线反馈优化机制，无法迭代改进"]}, "Standard Action-level RL": {"pros": ["理论框架成熟，可直接利用现有RL算法"], "cons": ["要求基础策略动作分布接近最优，对长horizon不适用", "探索空间大，样本效率低"]}}
method_comparison: "现有方法均在机器人动作层进行RL优化或固定提示，受限于VLA预训练分布；而SARL创新性地在语义提示空间进行强化学习，通过在线交互将提示与实际行为接地，实现了长horizon复杂任务的高效适配。"
method_debate_generated_at: "2026-09-01T12:13:23.104589Z"
