...
method_pros_cons: {"AD-CERT": {"pros": ["在多个认证基准上达到最先进认证精度", "结合教师引导下界替代与IBP上界，形成有效标量化", "比特征空间蒸馏提升最高5.40个百分点"], "cons": ["依赖预训练的实证鲁棒教师模型", "需联合优化蒸馏与IBP，计算复杂度较高"]}, "对抗训练": {"pros": ["实证鲁棒性强，标准准确率较高", "通过对抗样本扩充训练目标，优化最坏情况损失下界"], "cons": ["训练后的网络难以被形式化验证器认证", "面对更强或自适应攻击策略时表现不足"]}, "基于紧致松弛的认证训练": {"pros": ["训练网络易于形式化验证", "直接优化最坏情况损失的上界"], "cons": ["以牺牲标准准确率为代价", "在标准-认证精度权衡上表现较差"]}, "区间界传播（IBP）": {"pros": ["优化性质良好，易于在训练中应用", "提供最坏情况损失的松散上界近似"], "cons": ["单独使用时近似过于松散且不sound", "过度近似导致标准准确率下降"]}, "对抗训练与IBP结合的认证训练": {"pros": ["在标准-认证精度权衡上达到当前最优", "兼顾IBP优化友好性与对抗训练的实证优势"], "cons": ["仍依赖松散的IBP过近似", "方法选择与超参数敏感"]}, "对数级别对抗蒸馏": {"pros": ["比特征空间蒸馏更有效保持教师鲁棒性", "为认证训练提供有效的下界替代", "在统一设置下比特征空间蒸馏提升达5.40个百分点"], "cons": ["效果高度依赖教师模型的质量与鲁棒性", "需额外预训练和冻结教师网络"]}, "特征空间鲁棒蒸馏": {"pros": ["可在相同架构间传递鲁棒特征信息", "作为对比基线提供有价值的消融参考"], "cons": ["传递鲁棒性效果弱于对数级别蒸馏", "在认证训练中表现较差，差距最高5.40个百分点"]}}
method_comparison: "论文提出的AD-CERT通过将对抗训练的下界优势与IBP的上界保证相结合，在标准-认证精度权衡上优于纯紧致松弛认证训练和纯对抗训练，并在统一框架下验证了对数级别蒸馏优于特征空间蒸馏最高5.40个百分点。"
method_debate_generated_at: "2026-09-02T15:41:25.016979Z"
