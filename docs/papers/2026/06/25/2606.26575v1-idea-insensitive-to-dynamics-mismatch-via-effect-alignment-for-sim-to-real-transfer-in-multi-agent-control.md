...
method_pros_cons: {"IDEA（效果对齐方法）": {"pros": ["对动力学差异不敏感，提升鲁棒性", "真实场景成功率提升超20%，零碰撞", "训练效率高于主流迁移方法", "语义抽象层策略避免低层控制敏感问题"], "cons": ["依赖离散语义动作的合理设计", "需要并行仿真平台，计算资源门槛较高", "未在更多任务类型上验证泛化性"]}, "动作同步机制": {"pros": ["实现智能体间动作执行时间同步", "真实部署中动作时序误差为零", "通过通信增强系统时间一致性"], "cons": ["依赖可靠的智能体间通信基础设施", "通信延迟或丢包可能影响同步效果"]}, "基于Isaac Gym的并行训练平台": {"pros": ["支持大规模批量并行训练", "提供多样化几何结构环境", "在有限算力下实现高效训练"], "cons": ["依赖特定仿真引擎，移植性受限"]}, "Domain Randomization（域随机化）": {"pros": ["通过扰动动力学参数提升策略鲁棒性", "实现简单，无需真实数据", "是主流成熟的迁移方法"], "cons": ["难以覆盖非线性摩擦等真实特性", "对MARL非平稳性问题效果有限"]}, "Domain Adaptation（域自适应）": {"pros": ["利用真实数据适配策略或动力学模型", "可针对目标域做精细调整"], "cons": ["需真实环境交互数据，成本高", "复杂环境中适配过程脆弱"]}}
method_comparison: "IDEA通过将策略学习提升到语义抽象层并结合闭环控制，从根本上规避了动力学建模难题，相比Domain Randomization和Domain Adaptation在真实场景中实现了超过20%的成功率提升，且达到零碰撞和零时序误差。"
method_debate_generated_at: "2026-09-01T12:08:39.778677Z"
