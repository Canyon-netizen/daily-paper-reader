...
method_pros_cons: {"Psy-CoT": {"pros": ["心理学基础增强角色深度推理", "三步分解结构化思维过程", "在CoSER等基准上超越现有CoT方法"], "cons": ["推理步骤增加推理开销", "依赖心理学理论需专业标注", "数据合成质量影响最终性能"]}, "Interaction Perception": {"pros": ["基于心智理论推断对方意图", "提供全局场景感知能力"], "cons": ["推断准确性依赖模型能力", "多轮交互中可能累积误差"]}, "Psychological Empathy": {"pros": ["融合情感调节理论捕捉角色情感", "支持表达、掩饰、利用多种策略"], "cons": ["需角色背景知识完整", "情感识别可能产生主观偏差"]}, "Logical Construction": {"pros": ["基于目标行为理论合成角色世界观", "允许非理性或冲动决策"], "cons": ["短期目标选择可能偏离长期特征"]}, "RAPO": {"pros": ["互信息加权解决奖励破解问题", "多模型规模上一致超越GRPO", "非对称权重平衡探索与利用"], "cons": ["互信息计算增加训练开销", "依赖profile质量", "实现复杂度较高"]}, "SFT": {"pros": ["实现简单数据需求直接", "已有大量成熟方案如ChatHaruhi等"], "cons": ["仅模仿表面模式泛化能力差", "分布外角色退化为通用回复"]}, "通用CoT (General-purpose CoT)": {"pros": ["提供结构化推理基础", "通用性强易于应用"], "cons": ["引导模型追求客观最优解", "不适用于角色主观推理场景"]}, "GRPO": {"pros": ["已有成熟的RL优化框架", "实现相对简单"], "cons": ["对所有token平等对待", "通用与角色特定短语梯度信号相同"]}}
method_comparison: "Psy-CoT通过心理学三步推理替代通用CoT解决SFT的表面模仿问题，RAPO则通过互信息加权梯度解决GRPO的奖励破解问题，二者结合在多基准上均优于现有方法。"
method_debate_generated_at: "2026-09-01T12:09:08.588726Z"
