...
method_pros_cons: {"ATOD": {"pros": ["平均成功率比OPD高3.03、比GRPO高23.62点", "超越教师模型2.16点，突破模仿上限", "早期快速收敛，后期奖励驱动持续提升", "在ALFWorld/WebShop/Search-QA三任务均验证有效"], "cons": ["需联合调度OPD与RL两个目标，流程更复杂", "退火调度与T-DUR均引入额外超参数", "依赖教师模型的在线log-prob计算开销"], "cons_extra": ["仅在1.7B级小模型验证，大模型泛化待考"]}, "OPD": {"pros": ["教师token级分布提供密集监督，早期提升快", "训练稳定，不依赖环境奖励信号", "在小模型上比GRPO更高效（ALFWorld 72.7%）"], "cons": ["性能天花板受限于教师，接近后增益饱和", "强模仿可能压制偏离教师的奖励改进", "对长轨迹中关键决策点缺乏差异化处理"]}, "GRPO": {"pros": ["直接优化环境奖励，可超越教师水平", "鼓励探索性策略改进，理论上限更高", "无需教师模型即可独立训练"], "cons": ["稀疏延迟反馈，早期学习效率极低", "ALFWorld上仅10.2%，启动困难", "长视野任务中收敛慢、波动大"]}, "T-DUR": {"pros": ["将token级重要性提升到turn级，更贴合决策单元", "结合分歧度与熵双信号，覆盖四类高价值turn", "无需全词表教师采样，计算高效"], "cons": ["分歧度与熵估计可能受采样噪声影响", "权重设计依赖经验设定，缺乏理论保证", "仅在turn层级加权，未细化到token"]}, "Annealed OPD-RL Schedule": {"pros": ["前期OPD主导解决RL冷启动问题", "后期RL增强突破教师性能天花板", "显式利用OPD与RL互补性"], "cons": ["退火曲线与切换时机需额外调参", "调度策略在不同任务上未必通用", "两个目标联合优化增加训练不稳定性风险"]}, "SDAR": {"pros": ["作为agent训练基线提供对比参考", "在Search-QA上达到44.7%"], "cons": ["ALFWorld仅31.2%，远低于ATOD", "具体机制文中未深入讨论"]}, "SOD": {"pros": ["提供off-policy蒸馏对比基线", "WebShop上达76.6%具竞争力"], "cons": ["ALFWorld上仅39.8%，泛化不足", "缺乏奖励驱动改进机制"]}, "TCOD": {"pros": ["turn级对比基线，体现turn建模价值", "WebShop上81.2%表现较强"], "cons": ["ALFWorld上75.8%仍低于ATOD", "无RL奖励信号融合能力"]}}
method_comparison: "ATOD通过退火调度融合OPD的快速模仿与RL的奖励探索，并以T-DUR在turn层级加权蒸馏信号，在三个任务上同时超越纯OPD、纯GRPO及其他agent蒸馏基线（SDAR/SOD/TCOD），且突破教师性能天花板。"
method_debate_generated_at: "2026-09-01T12:09:34.067956Z"
