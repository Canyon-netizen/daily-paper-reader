...
method_pros_cons: {"Single-Agent Pipeline": {"pros": ["词法质量与MAS相当", "令牌消耗降低86%", "运行速度约为MAS两倍", "实现简单，无需多角色编排"], "cons": ["存在格式一致性问题", "缺乏任务分解的模块化优势", "宏观结构一致性低于MAS", "自主规划能力较弱"], "cons_translated": ["存在格式一致性问题", "缺乏任务分解的模块化优势", "宏观结构一致性低于MAS", "自主规划能力较弱"]}, "MAS (Multi-Agent System)": {"pros": ["结构一致性高达98%", "通过任务分解提升模块化", "迭代优化支持协作精炼", "适合复杂SE工作流分工"], "cons": ["令牌消耗显著更高", "运行速度较慢，约为单代理一半", "架构复杂，编排开销明显", "自主规划成为主要瓶颈"], "cons_translated": ["令牌消耗显著更高", "运行速度较慢，约为单代理一半", "架构复杂，编排开销明显", "自主规划成为主要瓶颈"]}, "DevPlan (Developer-guided Planning)": {"pros": ["整体文档质量超越所有对比方案", "轻量级开发者引导缓解规划瓶颈", "结合人工先验提升输出一致性", "弥补单代理自主规划不足"], "cons": ["依赖人工提供规划输入", "可扩展性受限于开发者参与", "增加前期标注成本", "削弱了端到端自动化优势"], "cons_translated": ["依赖人工提供规划输入", "可扩展性受限于开发者参与", "增加前期标注成本", "削弱了端到端自动化优势"]}, "LARCH": {"pros": ["当前README生成的SOTA基线", "启发式选择代表性代码片段", "基于显著代码元素提升质量", "提供稳定的对比基准"], "cons": ["依赖固定启发式规则", "采用单遍生成，缺乏迭代优化", "灵活性受限于预设策略", "未利用多代理协作优势"], "cons_translated": ["依赖固定启发式规则", "采用单遍生成，缺乏迭代优化", "灵活性受限于预设策略", "未利用多代理协作优势"]}, "RMGenie": {"pros": ["引入基于LLM代理的迭代框架", "支持工具调用与多步推理", "逐步精炼README内容", "分析仓库结构能力较强"], "cons": ["依赖多步推理，计算成本高", "可能引入迭代误差累积", "代理框架复杂度较高", "未在本文实验中对比评估"], "cons_translated": ["依赖多步推理，计算成本高", "可能引入迭代误差累积", "代理框架复杂度较高", "未在本文实验中对比评估"]}, "DocAgent": {"pros": ["多专业代理协作生成文档", "迭代推理提升事实准确性", "提升文档完整性", "支持验证机制减少错误"], "cons": ["代理协调增加系统开销", "事实准确性仍可能受限", "依赖多代理间通信质量", "未针对README任务专门优化"], "cons_translated": ["代理协调增加系统开销", "事实准确性仍可能受限", "依赖多代理间通信质量", "未针对README任务专门优化"]}, "Metagente": {"pros": ["多代理协作评估与精炼摘要", "在单代理基线上表现更优", "支持迭代质量提升", "面向README摘要任务设计"], "cons": ["仍依赖多代理架构成本", "评估范围限于摘要任务", "与MAS存在类似的规划瓶颈", "未在本文实验中纳入对比"], "cons_translated": ["仍依赖多代理架构成本", "评估范围限于摘要任务", "与MAS存在类似的规划瓶颈", "未在本文实验中纳入对比"]}, "ReadmeReady": {"pros": ["使用微调开源LLM", "结合代码索引生成基础内容", "避免对闭源模型的依赖", "部署成本相对可控"], "cons": ["生成内容为基础水平，质量有限", "微调成本与数据准备开销", "无法利用大模型涌现能力", "未在本文实验中对比评估"], "cons_translated": ["生成内容为基础水平，质量有限", "微调成本与数据准备开销", "无法利用大模型涌现能力", "未在本文实验中对比评估"]}, "RepoAgent": {"pros": ["利用仓库结构与文件间依赖", "提升文档覆盖范围", "捕获跨文件上下文信息", "适合仓库级文档生成"], "cons": ["依赖复杂的依赖分析", "对仓库结构质量敏感", "未在本文实验中对比评估", "工程实现复杂度较高"], "cons_translated": ["依赖复杂的依赖分析", "对仓库结构质量敏感", "未在本文实验中对比评估", "工程实现复杂度较高"]}}
method_comparison: "本文系统对比了Single-Agent Pipeline、专用MAS、DevPlan变体与LARCH基线在README生成任务上的表现，发现单代理在效率（省86%令牌、2倍速度）与MAS在质量（98%结构一致性）之间存在关键架构权衡，而轻量级DevPlan结合人工引导实现了最高整体文档质量。"
method_debate_generated_at: "2026-09-01T12:10:14.159195Z"
