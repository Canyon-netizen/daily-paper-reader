...
method_pros_cons: {"HRL-IM/CBS": {"pros": ["融合影响图哈希与聚类脚本，兼顾可解释性与适应性", "分层多Q表架构缓解联合动作维度爆炸", "通过奖励分配提供密集学习信号，提升样本效率", "在六个非对称场景中具有竞争力表现"], "cons": ["依赖手工设计的影响图哈希，泛化到新地图受限", "聚类策略与脚本需要领域知识设计", "面向星际争霸微观管理，跨领域迁移未验证", "未与最新大规模深度RL方法充分对比"]}, "Influence Map Hashing": {"pros": ["将全局战场压缩为紧凑十六进制编码，状态表征高效", "编码空间控制与相对优势，提升可解释性", "大幅降低状态空间维度，缓解维度爆炸"], "cons": ["哈希编码可能丢失精细空间信息", "离散编码设计需人工调参", "对地图布局变化敏感，泛化性存疑"]}, "Cluster-Based Scripts": {"pros": ["通过自适应单元划分实现动态局部协调", "相比固定编队更灵活应对战场变化", "脚本策略提供明确的战术意图可解释性"], "cons": ["脚本策略依赖领域先验知识", "组合爆炸可能导致聚类方案难以穷举", "面对高度复杂对抗时灵活性可能不足"]}, "Hierarchical Multi-Q-Table Architecture": {"pros": ["上层聚类策略选择与下层战术执行解耦，决策清晰", "分层结构有效分解复杂决策问题", "Q表表征透明，弥补深度模型黑盒缺陷"], "cons": ["多层Q表带来额外的训练调参复杂度", "层次间信息传递可能引入误差累积", "Q表存储需求随状态增长而膨胀"]}, "Deep RL Baselines": {"pros": ["端到端学习无需手工特征工程", "在大规模状态空间中具有强表达能力", "已在多种RTS任务中验证有效性"], "cons": ["稀疏奖励下样本效率低", "深度模型黑盒特性导致决策不透明", "联合动作空间维度爆炸难以处理"]}}
method_comparison: "HRL-IM/CBS通过分层架构与可解释表征在样本效率和透明度上优于深度RL基线，但以牺牲泛化性和需要领域知识设计为代价；影响图哈希、聚类脚本与多Q表三组件相互互补，共同应对维度爆炸与可解释性挑战。"
method_debate_generated_at: "2026-09-01T12:08:30.446520Z"
