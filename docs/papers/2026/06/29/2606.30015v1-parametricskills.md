...
title_zh: 参数化技能
authors: Xuan Zhao, Haonan He, Qingyu Yang, Minglei Li, Jingqi Ye, Zelin Tan, Bo Wan,
  Peng Ye
date: 2026-06-29
...
tldr: 智能体 LLM 的技能以文本形式存放在上下文中，在长上下文复杂任务中模型难以定位并遵循指令，能力受限。为此提出 ParametricSkills 框架，用超网络在测试时将文本技能转为
  LoRA 适配器参数，实现无上下文技能利用。该方法先构建技能库并用 OpenCode 合成单/多轮使用轨迹，再训练超网络同时编码技能内容与利用方式。在六项软件工程子任务上，由
  DeepSeek-V4-Flash 评判，平均比上下文学习高 6.44 分，BERT Score 与 F1 也显著领先，且参数化技能天然可累积，为测试时持续学习提供了初步途径。
source: arxiv
selection_source: web_analyzer
motivation: 文本形式的技能受限于模型理解与遵循复杂指令的能力，在长上下文中关键指令难以定位，技能利用效果受限，且技能演化与模型学习相解耦。
method: 提出 ParametricSkills 框架，构建大规模技能库并用 OpenCode 合成单轮与多轮技能利用轨迹，训练一个超网络，将文本技能映射为
  LoRA 适配器参数，实现测试时无上下文技能利用。
result: 在六项复杂软件工程子任务上，ParametricSkills 经 DeepSeek-V4-Flash 评判平均比上下文学习高出 6.44 分，BERT
  Score 与 F1 也显著更高。
conclusion: 参数化技能天然具备可累积性，初步展示了测试时持续学习的可行路径，并为技能全生命周期管理提供了新方向。
figures_json: '[{"url": "assets/figures/arxiv/2606.30015v1/fig-001.webp", "caption":
  "", "page": 0, "index": 1, "width": 1737, "height": 2217}, {"url": "assets/figures/arxiv/2606.30015v1/fig-002.webp",
  "caption": "", "page": 0, "index": 2, "width": 2052, "height": 1124}, {"url": "assets/figures/arxiv/2606.30015v1/fig-003.webp",
  "caption": "", "page": 0, "index": 3, "width": 2052, "height": 1124}, {"url": "assets/figures/arxiv/2606.30015v1/fig-004.webp",
  "caption": "", "page": 0, "index": 4, "width": 2052, "height": 1124}, {"url": "assets/figures/arxiv/2606.30015v1/fig-005.webp",
  "caption": "", "page": 0, "index": 5, "width": 1734, "height": 912}, {"url": "assets/figures/arxiv/2606.30015v1/fig-006.webp",
  "caption": "", "page": 0, "index": 6, "width": 1737, "height": 2076}]'
wiki_compiled: true
wiki_compiled_at: 2026-07-22 08:22:28.500185+00:00
concepts:
- slug: parametric-skills
  display_name: 参数化技能
  category: method
  novelty: 0.95
  centrality: 1.0
...
- slug: few-shot-prompting
  ...
  novelty: 0.0
  centrality: 0.6
- slug: test-time-continual-learning
  display_name: 测试时持续学习
  category: problem
  novelty: 0.5
  centrality: 0.55
- slug: agent-skill
  ...
  novelty: 0.2
  centrality: 0.7
- slug: software-engineering-task
  display_name: 软件工程任务
  category: problem
  novelty: 0.0
  centrality: 0.5

method_pros_cons: {"ParametricSkills": {"pros": ["相比ICL平均提升6.44分，优势显著", "实现上下文无关技能利用，避免长文本寻址", "技能以参数形式累积，支持测试时持续学习", "将技能进化与模型学习耦合，更内生化"], "cons": ["依赖超网络与LoRA训练，工程复杂度较高", "参数量受LoRA秩限制，表达能力有上限", "训练数据依赖合成轨迹，可能存在分布偏差"]}, "In-Context Learning (ICL)": {"pros": ["无需额外训练，部署成本极低", "灵活性高，可即时增删技能文本", "兼容现有开源模型，无需修改参数"], "cons": ["依赖模型理解与遵循复杂指令能力", "长上下文中关键指令难以定位遵循", "性能随模型规模减小急剧下降"]}, "EvoSkill/CoEvoSkills/SkillOpt (文本空间技能进化方法)": {"pros": ["不更新模型参数，部署门槛低", "通过失败分析与验证器迭代改写技能", "在多种下游任务上验证有效"], "cons": ["技能进化与模型学习解耦，效果受限", "技能仍以文本形式存在，受限于上下文窗口", "模型本身未被优化，上限受基座能力制约"]}, "Text-to-LoRA/LoRAGen (超网络生成任务级LoRA)": {"pros": ["超网络一次训练可生成多任务适配器", "大幅降低下游微调计算与存储成本", "参数生成高效灵活"], "cons": ["仅针对任务级适配，未涉及技能级利用", "未解决长上下文场景的指令遵循瓶颈", "泛化到未见任务的能力有待验证"]}, "Doc-to-LoRA/SHINE (超网络压缩上下文为LoRA)": {"pros": ["消除KV缓存存储开销，节省显存", "实现上下文的参数化压缩与高效复用", "支持超长文档的参数化处理"], "cons": ["主要用于文档压缩，非面向技能利用", "压缩保真度受超网络容量限制", "未涉及技能的持续进化机制"]}}
method_comparison: "ParametricSkills通过超网络将文本技能转为LoRA参数，相比ICL和文本空间技能进化方法（EvoSkill等）性能显著更优且支持持续学习，但在工程复杂度和表达能力上做出权衡；相比Text-to-LoRA和Doc-to-LoRA等超网络方法，ParametricSkills将应用场景从任务级或上下文级扩展到技能全生命周期。"
method_debate_generated_at: "2026-09-01T12:08:20.887227Z"
