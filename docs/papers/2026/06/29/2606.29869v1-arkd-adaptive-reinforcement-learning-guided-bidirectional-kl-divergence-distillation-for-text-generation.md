...
method_pros_cons: {"ARKD（自适应强化学习双向KL蒸馏）": {"pros": ["策略网络动态分配FKL/RKL权重，实现状态自适应", "在Rouge-L和BertScore上超越贪心启发式0.4-0.6分", "同时对齐主模式和长尾模式，缓解长尾过估计", "具备理论分析支撑FKL/RKL互补性与收敛一致性"], "cons": ["引入策略网络增加训练复杂度和超参数", "依赖即时奖励信号设计，奖励工程难度大", "实验仅基于摘要与部分正文，泛化性待充分验证"]}, "前向KL散度（FKL）": {"pros": ["模式覆盖行为，能覆盖教师分布全部模式", "理论成熟，是知识蒸馏的经典目标函数", "实现简单，计算开销低"], "cons": ["在低置信长尾区域过估计概率，损害生成质量", "模式覆盖导致学生分散，忽略主导模式聚焦", "对分布外样本泛化能力不足"]}, "反向KL散度（RKL）": {"pros": ["模式寻求行为，避免长尾低概率区域过拟合", "聚焦教师主导模式，提升主分布拟合质量", "MiniLLM等已证明其在LLM蒸馏中的有效性"], "cons": ["牺牲多样性和覆盖率，忽略次要模式", "过度集中于单一主导模式，长尾建模不足", "静态使用无法适应训练中学生-教师差距的变化"]}, "静态/贪心启发式插值方法": {"pros": ["实现简单，无需额外策略网络", "在FKL和RKL间提供基础平衡"], "cons": ["固定系数无法捕捉训练中学生-教师差距的动态变化", "性能低于ARKD自适应方法0.4-0.6分", "缺乏对分布特征的自适应调整能力"]}, "序列级蒸馏（黑盒）": {"pros": ["无需访问教师内部表示，部署灵活", "仅用教师生成文本即可训练学生模型"], "cons": ["缺乏token级分布对齐，精度低于白盒方法", "无法利用教师logits中的细粒度分布信息"]}, "Token级蒸馏（白盒）": {"pros": ["利用教师logits进行细粒度分布对齐", "在LLM压缩和部署中已被证明有效", "可灵活选择FKL或RKL作为对齐目标"], "cons": ["需要访问教师内部表示，部署受限", "单一KL目标难以兼顾主分布与长尾建模"]}}
method_comparison: "本文对比了FKL（模式覆盖）、RKL（模式寻求）及静态插值方法，指出单一目标或固定权重均无法兼顾主分布拟合与长尾建模；所提ARKD通过强化学习策略网络动态加权FKL/RKL，实现优于贪心启发式0.4-0.6分的双重分布对齐。"
method_debate_generated_at: "2026-09-01T12:07:40.045522Z"
