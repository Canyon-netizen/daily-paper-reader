...
method_pros_cons: {"DRIFT": {"pros": ["五基准平均79.5%，超越GRPO 9.5%、SDPO 7.5%", "ToolUse达79.2%，显著优于所有并发方法", "联合难度路由与节奏门控，自适应信号分配", "无需外部专家监督即可稳定自进化"], "cons": ["框架含多组件，调参复杂度较高", "主要在Qwen3-8B系列验证，泛化待考", "依赖历史通过率估计，冷启动需预热", "论文15页，消融实验深度有限"]}, "Difficulty Routing（难度路由）": {"pros": ["基于历史通过率稳定估计掌握度", "按难度动态分配蒸馏与强化信号", "避免已掌握题冗余更新", "为难题提供历史成功轨迹参考", "针对边界题强化探索"], "cons": ["需累积批次数据估计通过率", "硬题蒸馏依赖缓冲区历史解质量", "难度划分阈值需额外调参"]}, "Rhythm Gating（节奏门控探索）": {"pros": ["在token层级精化策略更新", "聚焦关键推理位置施加结构化探索奖励", "保留有效推理模式不被破坏"], "cons": ["增加策略更新机制复杂度", "关键位置识别依赖启发式设计", "可能引入额外超参"]}, "Success Buffer（成功缓冲区）": {"pros": ["跨批次复用历史正确轨迹", "为当前批未解难题提供蒸馏参考", "形成持久自进化记忆"], "cons": ["带来存储与检索开销", "缓冲区旧解可能与当前能力失配", "需额外管理机制防过拟合"]}, "Two-Stage Curriculum Learning（两阶段课程学习）": {"pros": ["对齐模型不同学习阶段行为", "先快速能力增长后稳定收敛", "兼顾效率与稳定性"], "cons": ["阶段切换时机需精心设计", "两阶段间过渡可能不平滑"]}, "GRPO": {"pros": ["无需critic的相对奖励机制", "鼓励策略探索，已被广泛验证", "实现相对简单，训练稳定"], "cons": ["成功样本稀疏时更新噪声大", "奖励分布不稳定时训练波动", "无题目级难度感知，信号分配欠佳", "难题监督弱，易过度优化简单题"]}, "SDPO": {"pros": ["复用模型自身正确解作为监督", "减少对人类专家演示依赖", "自蒸馏方法已证明有效"], "cons": ["对所有正确解均匀处理，忽视难度差异", "无法区分易题与边界题", "缺乏问题级学习进度追踪", "在难题上监督信号弱"]}, "SRPO": {"pros": ["结合自蒸馏与强化学习的样本路由方法", "近期改进方向已超越纯RL基线"], "cons": ["仅依赖当前批次即时结果", "缺乏持久难度与学习进度刻画", "信号分配仍非最优"]}}
method_comparison: "DRIFT通过问题级难度路由和token级节奏门控联合机制，同时解决了GRPO更新噪声大和SDPO均匀处理的局限，在五个基准上全面超越两者及SRPO等并发方法。"
method_debate_generated_at: "2026-09-01T12:09:12.037979Z"
