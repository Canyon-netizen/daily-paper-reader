...
method_pros_cons: {"Deep Reinforcement Learning (DRL)": {"pros": ["可学习复杂序列决策行为", "已用于机器人与自主系统"], "cons": ["对微小对抗扰动高度敏感", "可能产生不安全控制行为"]}, "RoAd-RL": {"pros": ["统一策略攻击防御和指标", "提供可复现评估流程", "集成192种攻防配置", "兼容两个主流强化学习库"], "cons": ["实验仅涉及两个基准环境", "跨环境鲁棒性差异显著"]}, "Policy abstraction": {"pros": ["统一封装多类强化学习策略", "便于跨算法比较鲁棒性", "新策略无需修改基础设施"], "cons": ["统一接口可能限制特有能力", "第三方策略仍需适配"]}, "Attack abstraction": {"pros": ["统一组合多种攻击方法", "支持跨扰动预算比较", "新攻击可直接复用流程"], "cons": ["接口难覆盖全部威胁模型", "攻击效果依赖环境与预算"]}, "Defense abstraction": {"pros": ["便于独立替换防御机制", "支持多种攻防自动组合", "新防御无需修改底层流程"], "cons": ["组合评估增加计算开销", "防御收益具有环境依赖性"]}, "Metric abstraction": {"pros": ["集中实现鲁棒性指标", "支持跨预算和种子比较", "便于产生标准化结果"], "cons": ["指标选择会影响结论", "单一指标难覆盖安全性"]}, "192种攻防配置评估协议": {"pros": ["覆盖两个环境与三种算法", "系统比较192种攻防组合", "采用多扰动预算和随机种子"], "cons": ["配置规模增加评估成本", "仅两个环境限制结果外推"]}, "DQN": {"pros": ["可作为对抗鲁棒性基线", "已有明确FGSM脆弱性证据"], "cons": ["小扰动可显著改变动作", "对观测攻击较为敏感"]}, "PPO": {"pros": ["代表常用策略梯度算法", "纳入多算法鲁棒性比较"], "cons": ["论文未逐项报告其结果", "性能排序受环境明显影响"]}, "SAC": {"pros": ["代表常用连续控制算法", "纳入多算法鲁棒性比较"], "cons": ["论文未逐项报告其结果", "性能排序受环境明显影响"]}, "TRPO": {"pros": ["参与早期对抗脆弱性验证", "可检验FGSM跨算法影响"], "cons": ["未纳入本次192种配置", "本文未报告其独立结果"]}, "A3C": {"pros": ["参与早期对抗脆弱性验证", "可检验FGSM跨算法影响"], "cons": ["未纳入本次192种配置", "本文未报告其独立结果"]}, "FGSM": {"pros": ["单步生成扰动且计算高效", "已能显著降低多种算法回报"], "cons": ["单步攻击通常弱于PGD", "未必覆盖最坏攻击情形"]}, "PGD": {"pros": ["迭代寻优使攻击更强", "在强化学习中已实证有效"], "cons": ["多步优化增加计算成本", "结果受扰动预算影响"]}, "JSMA": {"pros": ["仅扰动少量关键特征", "小范围修改即可影响决策"], "cons": ["适用性受观测结构限制", "本文未提供独立量化结果"]}, "Temporally Consistent Attacks": {"pros": ["跨时间步保持扰动一致性", "更难检测且保留攻击效果"], "cons": ["联合优化增加实现复杂度", "本文未提供独立量化结果"]}, "Reward Poisoning": {"pros": ["可攻击奖励而非观测信号", "扩展训练阶段威胁模型"], "cons": ["与测试时观测攻击不同", "未纳入RoAd-RL配置"]}, "Environment Manipulation": {"pros": ["可操纵环境交互过程", "扩展交互层攻击面"], "cons": ["与观测攻击难以直接比较", "未纳入192种公开配置"]}, "Policy Poisoning": {"pros": ["可攻击策略获取过程", "补充训练后安全评估"], "cons": ["评估条件不同于测试攻击", "现有实验未覆盖该机制"]}, "Adversarial Policy Training": {"pros": ["扩展观测扰动之外的训练范式", "可研究攻击与策略联合优化"], "cons": ["训练成本通常更高", "未纳入192种评估配置"]}, "Adversarial Training": {"pros": ["训练期暴露于对抗样本", "是广泛采用的防御方法"], "cons": ["增加训练阶段计算开销", "防御收益受攻击质量影响"]}, "Feature Squeezing": {"pros": ["压缩输入可降低扰动自由度", "易于作为防御模块接入"], "cons": ["可能损失任务观测细节", "本文未报告单独收益数据"]}, "Temporal Smoothing": {"pros": ["跨时间平滑提升策略鲁棒性", "实验中持续表现强劲"], "cons": ["可能引入动作响应延迟", "效果依赖平滑窗口设置"]}, "Randomized Smoothing": {"pros": ["随机化可降低固定扰动影响", "可作为独立防御模块接入"], "cons": ["随机采样增加评估开销", "本文未报告单独收益数据"]}, "Anomaly Detection": {"pros": ["可识别异常观测或行为", "提供独立攻击检测防线"], "cons": ["检测阈值需要环境校准", "对时序一致攻击可能漏检"]}, "RobustBench": {"pros": ["标准化监督模型鲁棒评估", "提供成熟的可复现基准经验"], "cons": ["不能直接统一时序交互", "仍需强化学习专用适配"]}, "Foolbox": {"pros": ["集中提供多种攻击工具", "减少攻击实现碎片化"], "cons": ["主要面向监督学习模型", "时序状态需要额外扩展"]}, "CleverHans": {"pros": ["提供标准对抗攻击实现", "支持模型鲁棒性复现"], "cons": ["主要面向监督学习模型", "时序策略需要额外扩展"]}, "Stable-Baselines3": {"pros": ["提供多种常用算法实现", "便于替换统一策略后端"], "cons": ["本身不提供对抗安全保证", "复杂攻防机制仍需扩展"]}, "Gymnasium": {"pros": ["统一环境状态与动作接口", "支持两个基准环境复现"], "cons": ["接口标准不等于策略鲁棒", "环境差异会改变攻防结论"]}}
method_comparison: "RoAd-RL借助四类抽象和192种攻防配置统一了评估流程，但跨环境鲁棒性差异明显。相较单步FGSM，迭代PGD通常更强，时序一致攻击也更难检测；实验中Temporal Smoothing持续表现强劲，而部分常见防御甚至比攻击更有害。"
method_debate_generated_at: "2026-09-01T12:30:20.750819Z"
