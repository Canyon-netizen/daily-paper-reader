...
method_pros_cons: {"Multi-Agent Debate with Majority Voting": {"pros": ["广泛用于提升LLM推理能力的成熟范式", "通过多轮结构化讨论可拓展Self-Consistency", "实现简单，无需修改底层LLM权重"], "cons": ["依赖Condorcet Jury Theorem的独立误差假设", "LLM共享预训练语料导致误差强相关", "多数票系统压制少数派正确意见（Minority Truth）", "分歧样本中25.5%少数派实际正确却被打压"]}, "Minority Sentinel": {"pros": ["诊断-治疗双阶段框架，无需修改LLM权重", "使用LightGBM非LLM分类器实现认知正交性", "Flip Precision达81.2%，六数据集全为正Net Gain", "插即用，仅在投票聚合层插入轻量安全阀"], "cons": ["依赖辩论日志中的行为特征，可解释性有限", ["仅假设三智能体场景，更多智能体待验证"], "LightGBM需训练阶段，特征工程依赖人工设计", "恢复能力受限于10百分点的理论上限"]}, "LLM-as-Judge": {"pros": ["语义级理解能力强，可提供详细判断依据", "广泛应用于评估任务，泛用性较高", "与多智能体辩论结合可提升判断鲁棒性"], "cons": ["法官LLM与辩论智能体共享认知盲点", "读辩论日志裁决产生负Net Gain，劣于不干预", ["高Recall但Flip Safety差，干预价值低"], "无法突破LLM认知闭环的根本限制"]}}
method_comparison: "MAD多数投票因LLM误差相关性系统性压制正确少数意见；LLM-as-Judge因认知同质化产生负净收益；Minority Sentinel以非LLM分类器利用行为信号打破认知闭环，在Flip Safety上显著优于LLM法官"
method_debate_generated_at: "2026-09-01T12:01:21.136054Z"
