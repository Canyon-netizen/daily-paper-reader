...
method_pros_cons: {"RESOURCE2SKILL": {"pros": ["多模态资源融合，覆盖教程视频、代码库、文章等", "平均得分提升+11.9个百分点", "在28个模型-领域单元中26个超越基线", "支持在线获取新技能，动态扩展能力"], "cons": ["依赖高质量多模态资源，资源获取成本高", "Skill Wiki维护与检索开销较大", "摘要分析，缺少具体消融实验细节", "未详述对未知领域的泛化能力"]}, "Skill Wiki": {"pros": ["层级化组织，结构清晰便于检索组合", "融合文本、代码、视觉示例、元数据和溯源", "保留视频时序操作与视觉效果信息", "支持多源互补信号的结构化存储"], "cons": ["层级结构可能引入检索粒度选择难题", "元数据与溯源信息增加存储负担", "跨条目知识组合的冲突需额外处理", "视觉示例可能带来较大的存储开销"]}, "多模态技能蒸馏": {"pros": ["充分利用教程视频等被忽视的多模态资源", "将人类经验转化为可执行程序性知识", "保留视觉操作与代码可执行模式互补", "融合概念风格与结构化操作指导"], "cons": ["蒸馏质量依赖源资源标注质量", "视频时序信息到代码的映射复杂", "摘要层面未提供具体实现细节", "不同模态对齐精度可能影响效果"]}, "在线技能获取": {"pros": ["覆盖不足时动态补充新技能", "复用同一构造算子，降低实现复杂度", "提升框架自适应与持续学习能力", "解决预建技能库覆盖面有限问题"], "cons": ["在线获取延迟可能影响推理实时性", "新技能质量验证机制尚不明确", "依赖检索质量，错误累积风险存在", "在线检索的算力与成本未详细分析"]}, "No-skill Agents基线": {"pros": ["作为零能力参考基线，量化技能贡献", "实现简单，计算开销低", "提供清晰的下界性能基准"], "cons": ["缺乏可复用知识，复杂任务表现差", "无法利用人类经验解决专业问题", "在7个领域平均得分低于RESOURCE2SKILL 11.9点"]}, "Harness Baselines": {"pros": ["代表强基线，提供更具挑战性的对比", "在多个模型与领域组合下被超越", "验证框架跨域泛化能力"], "cons": ["基线仅在26/28单元被超越，存在短板", "摘要未详述其具体实现细节"]}}
method_comparison: "RESOURCE2SKILL通过多模态技能蒸馏和层级化Skill Wiki，相比无技能基线平均提升11.9个百分点，在28个模型-领域单元中26个超越Harness基线，验证了多模态融合与在线获取策略的有效性"
method_debate_generated_at: "2026-09-01T11:54:34.214588Z"
