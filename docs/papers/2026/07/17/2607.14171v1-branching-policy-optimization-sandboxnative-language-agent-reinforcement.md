...
method_pros_cons: {"Branching Policy Optimization (BPO)": {"pros": ["沙盒原生设计，利用快照恢复特性", "理论证明无偏且方差严格低于GRPO", "匹配算力下成功率提升3.6–6.1绝对点", "梯度方差减半，更新步数减少38%"], "cons": ["依赖沙盒快照能力，通用性受限", "分支调度与快照开销增加工程复杂度", "仅在三类沙盒基准上验证", "熵驱动分支策略需调参"]}, "GRPO": {"pros": ["无需学习critic，内存开销低", "组基线简单有效", "在agent RL中已成为主流算法"], "cons": ["基线仅以初始状态为条件", "独立采样N条轨迹方差极大", "长程任务中早期错误被放大"]}, "RLOO": {"pros": ["REINFORCE风格实现简洁", "无需critic网络，降低内存", "与GRPO同属基线类方法"], "cons": ["沿用RLHF独立轨迹拓扑", "长程任务方差控制不足", "未利用沙盒可恢复特性"]}, "PPO": {"pros": ["RLHF部署的事实标准", "裁剪目标稳定策略更新", "广泛适用各类RL场景"], "cons": ["需要学习critic，内存成本高", "在LLM规模下critic常非必要", "基线仅依赖初始prompt"]}, "VinePPO": {"pros": ["用蒙特卡洛替代学习critic", "从中间状态回滚估计价值", "与BPO精神最接近"], "cons": ["未改变rollout拓扑结构", "未利用沙盒快照原语", "分支策略非自适应"]}, "DPO": {"pros": ["无需在线采样，训练简单", "直接优化偏好，避免RL不稳定"], "cons": ["仅适用于完整轨迹偏好场景", "无法用于agent沙盒交互任务"]}, "STaR": {"pros": ["循环利用on-policy成功样本", "通过迭代SFT提升能力"], "cons": ["非RL方法，不优化奖励信号", "依赖已有成功轨迹的可用性"]}, "Tree-of-Thoughts / RAP / AlphaZero风格树搜索": {"pros": ["推理时利用状态快照进行树搜索", "探索更丰富的决策空间"], "cons": ["仅用于推理而非训练阶段", "未将检查点恢复作为训练原语"]}}
method_comparison: "BPO通过共享前缀的树状rollout拓扑，利用沙盒快照恢复实现更低方差的优势估计，在匹配算力下显著优于GRPO、RLOO、PPO和VinePPO等基线类方法，是首个将checkpoint-restore作为一等训练原语的sandbox-native RL算法。"
method_debate_generated_at: "2026-09-02T15:58:47.844875Z"
