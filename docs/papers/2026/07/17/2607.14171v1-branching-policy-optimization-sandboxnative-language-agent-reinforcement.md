...
强化学习在训练大语言模型智能体时通常为每个提示独立采样多条轨迹，但忽视了沙箱环境的一个重要特性——它可以随时快照并从任意中间状态恢复。研究者据此提出分支策略优化（BPO），通过在同一棵树上共享前缀轨迹来复用计算、压缩方差。在 WebShop、ALFWorld 和 SWE-bench Verified 三个基准上，BPO 将成功率绝对提升 3.6–6.1 个百分点，同时将梯度范数方差减半，并以减少 38% 的策略更新次数达到与最佳基线相当的效果。
