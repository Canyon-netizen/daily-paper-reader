...
method_pros_cons: {"Privileged On-Policy Self-Distillation (OPSD)": {"pros": ["对指令调优模型有帮助", "教师可利用测试时推理吸收特权信息", "教师条件化金解时保持高pass@k", "实现简单，学生复用同架构"], "cons": ["使思维模型avg@16相对下降达17%", "在长rollout预算下退化最严重", "在高熵分叉位置翻转token优势信号", "抑制验证、回溯、犹豫等审议行为"]}, "Vanilla On-Policy Distillation (OPD)": {"pros": ["能提升思维模型推理能力", "在分叉token上保持正向优势", "支持学生自主修正与重考虑", "短预算下训练仍可获得增益"], "cons": ["对强思维模型提升幅度有限", "现有研究多在思维关闭下评估", "未充分考虑token级微调信号", "依赖教师与学生质量差距"]}, "On-Policy Self-Distillation (OPSD) 整体范式": {"pros": ["模型可同时充当师生，部署简便", "思维模型是天然自改进候选", "无需额外标注数据即可迭代", "可利用测试时审议充分吸收信息"], "cons": ["先前研究多限于短生成长度", "多在指令模型上验证有效性", "特权上下文会损害强思维模型", "需关注分叉位置的token级信号"]}, "Fork Suppression (分叉抑制现象)": {"pros": ["清晰解释特权自蒸馏退化机制", "揭示特权上下文降低思维模型分叉率", "指出自修正分支中token信号反转", "为token级诊断提供可分析框架"], "cons": ["仅在思维模型而非指令模型出现", "随rollout预算增大而加剧", "导致审议标记减少且不可逆", "说明现有自蒸馏方法需重新设计"]}}
method_comparison: "论文对比了Vanilla OPD与特权上下文OPD在思维模型上的表现，发现特权上下文会翻转分叉位置的token优势信号，导致fork suppression现象，使思维模型在长rollout预算下退化高达17%，揭示了特权自蒸馏在指令模型上有效但在强思维模型上有害的关键差异。"
method_debate_generated_at: "2026-09-01T12:36:21.861015Z"
