...
method_pros_cons: {"CompactionRL": {"pros": ["联合优化任务执行与摘要生成，提升压缩质量", "在SWE-bench Verified上提升5.5-7.0个绝对点", "在Terminal-Bench 2.0上提升3.1-6.8个绝对点", "上下文预算固定，训练可超越峰值上下文长度"], "cons": ["仅在编程任务上验证，泛化性待考察", "依赖critic估计，训练复杂度更高", "摘要质量影响后续所有动作，信息丢失风险"]}, "Context Compaction（推理时启发式）": {"pros": ["实现简单，无需额外训练成本", "天然支持长轨迹的继续执行", "已有广泛的语言代理记忆系统参考"], "cons": ["压缩策略固定，无法针对任务优化", "摘要质量不可控，影响任务成功率", "被视为外部启发式，缺乏学习信号"]}, "GRPO": {"pros": ["无需critic，训练效率较高", "组内归一化优势简单有效", "已在多种LLM RL任务中验证有效"], "cons": ["假设完整rollout可处理，不支持压缩分段", "固定大小组归一化在变长分段下不适配", "无法处理压缩边界处的信用分配"]}, "PPO": {"pros": ["通过critic支持token级优势估计", "可处理变长压缩分段训练", "信用分配机制成熟，适合长horizon"], "cons": ["需额外训练critic，计算开销更大", "训练稳定性对超参敏感", "实现复杂度高于GRPO"]}, "Token级损失归一化": {"pros": ["减少变长分段引入的权重偏差", "保证不同长度段落的公平学习", "简单有效，无需额外模型"], "cons": ["仅缓解长度偏差，不能解决信息丢失", "归一化策略可能掩盖真实难度差异"]}, "跨轨迹GAE": {"pros": ["保留压缩边界间的时序信用分配", "在分段训练中保持优势估计一致性", "与token级归一化互补，增强训练稳定"], "cons": ["跨轨迹边界处优势估计可能存在噪声", "依赖critic质量，增加训练成本"]}}
method_comparison: "CompactionRL在PPO框架下联合训练任务执行与摘要生成，通过token级损失归一化和跨轨迹GAE解决变长压缩分段的训练难题，相比推理时压缩启发式和GRPO在长horizon代理任务上取得显著性能提升。"
method_debate_generated_at: "2026-09-01T12:37:13.401644Z"
