...
method_pros_cons: {"Entity Graph建模方法": {"pros": ["将运营实体作为节点而非智能体或静态基础设施", "通过异构边编码竞争、协作与连接关系", "提供关系归纳偏置捕捉部分可观测环境", "适配铁路动态定价中多运营商市场交互"], "cons": ["图结构设计依赖领域知识，增加建模复杂度", "异构边类型增加计算与调参开销", "关系定义需根据市场结构手动调整"]}, "R-GCN + 注意力机制的MATD3扩展": {"pros": ["多层R-GCN有效传播实体间关系信息", "注意力机制选择性聚合战略相关特征", "梯度停止操作解耦表征学习与策略优化", "相比基线实现更高收入与训练稳定性"], "cons": ["梯度停止可能降低部分场景下的总收入", "图表示学习增加训练阶段的计算成本", "在去中心化执行时图嵌入不可用"]}, "MATD3算法": {"pros": ["双Q网络与延迟更新减少TD3高估偏差", "支持连续动作空间契合动态定价决策", "CTDE范式兼顾训练全局性与执行隐私性"], "cons": ["原始版本忽略实体间拓扑结构信息", "将观测视为非结构化向量，丧失关系信息"]}, "车站基图建模（Station-based Graph）": {"pros": ["广泛用于铁路网络拓扑分析与脆弱性评估", "直观表达轨道物理连接关系", "成熟范式，适合容量规划等任务"], "cons": ["以车站为节点，无法捕捉运营动态", "不适用于服务差异化与多运营商竞争", "缺乏市场层面的策略交互建模能力"]}, "CTDE范式": {"pros": ["训练阶段可访问全局信息提升学习效果", "执行阶段保持去中心化策略保护隐私", "契合监管禁止运营商间信息交换的约束"], "cons": ["训练时依赖集中式信息，部署环境受限", "全局信息与个体观测间存在表征差异"]}, "R-GCN（关系图卷积网络）": {"pros": ["原生处理异构图，区分多种关系类型", "通过消息传递自适应动态变化的拓扑", "无需修改架构即可适应关系演变"], "cons": ["层数增加易导致过度平滑问题", "参数随关系类型数量增长而增加"]}, "注意力聚合机制": {"pros": ["允许智能体个性化筛选相关实体信息", "动态优先排序战略相关的服务特征", "生成紧凑且个性化的环境状态表示"], "cons": ["注意力权重引入额外计算开销", "学习到的注意力模式可解释性有限"]}}
method_comparison: "本文提出以运营实体为节点的Entity Graph建模方法，并将其通过R-GCN与注意力机制嵌入MATD3算法，相较于传统的车站基图范式与原始非关系型MARL基线，在铁路动态定价场景下能更好地捕捉市场拓扑与策略依赖，实现更优的收入与训练稳定性。"
method_debate_generated_at: "2026-09-02T05:37:16.479188Z"
