...
method_pros_cons: {"TACO (Tail-Aware Credit Calibration)": {"pros": ["从局部语义视角校准token级信用分配", "几乎不增加额外计算开销", "在三个LLM和八个基准上一致优于GRPO", "改善长horizon训练稳定性"], "cons": ["tail-risk score估计质量影响整体效果", "主要在数学与科学推理基准验证", "对negative credit校准讨论较少"]}, "GRPO (Group Relative Policy Optimization)": {"pros": ["组相对优势替代学习critic减少开销", "简单有效，广泛采用为推理RL骨干", "实证性能强，工程生态成熟"], "cons": ["均匀信用分配，不区分token可靠性", "存在Positive-Credit Contamination失效模式", "低概率tail token获相同正向信用"]}, "DAPO (Decoupled Clip and Dynamic Sampling Policy Optimization)": {"pros": ["解耦clip机制和动态采样策略", "改进GRPO的采样效率与稳定性"], "cons": ["仍基于均匀信用分配", "未解决tail token的正向污染问题"]}, "GSPO (Group Sequence Policy Optimization)": {"pros": ["组序列级策略优化改进GRPO", "对序列粒度信用分配有改进"], "cons": ["仍受token级均匀信用问题困扰", "未直接处理implausible tail token"]}, "PPO (Proximal Policy Optimization)": {"pros": ["经典RL方法，通过learned critic估计优势", "理论基础扎实，性能可靠"], "cons": ["需要学习value model，开销较大", "critic学习增加训练复杂度和成本"]}, "基于外部信号的token信用方法 (counterfactual/TD/execution feedback)": {"pros": ["利用外部信号精确评估每个token贡献", "可精确缩放token级信用大小"], "cons": ["依赖额外推理或辅助模型，开销高", "可能将不可靠信用错误放大", "资源开销限制实际部署"]}, "基于内在信号的token信用方法 (entropy/distributional divergence)": {"pros": ["利用entropy等内在信号无需外部模型", "聚焦推理过程关键token的更新"], "cons": ["缺乏局部语义视角", "可能误判高entropy tail token为重要"]}}
method_comparison: "本文以TACO与GRPO为核心对比，指出GRPO族方法因均匀信用分配存在Positive-Credit Contamination问题，而TACO通过局部tail-risk评分实现细粒度校准，在效率和效果上均优于基于外部信号和基于内在信号的现有token级信用方法。"
method_debate_generated_at: "2026-09-02T05:42:26.877561Z"
