...
method_pros_cons: {"AcoustoBots": {"pros": ["实现动态可移动的数据物理化平台", "支持单/双机器人两种任务模式", "单机器人任务成功率90%", "实现感知-显示-动作闭环"], "cons": ["粒子悬浮高度仅1-10cm", "实验场地限于4m×3m缩放地图", "每种模式仅10次试验，统计有限"]}, "MADDPG (Multi-Agent Deep Deterministic Policy Gradient)": {"pros": ["集中训练分散执行支持协同导航", "适用于连续动作空间的避障决策", "可实现碰撞感知的运动控制"], "cons": ["集中训练计算开销较大", "双机器人场景成功率降至80%", "策略扩展性受限于集中式critic"]}, "GS-PAT (Gerchberg-Saxton Phased Array of Transducers)": {"pros": ["高速相位更新保障运动中陷阱稳定", "精确控制粒子高度编码城市标量值", "实现8×8阵列的多粒子独立操控"], "cons": ["Gerchberg-Saxton迭代计算复杂度高", "垂直调控范围受限(1-10cm)", "对实时相位计算要求严格"]}}
method_comparison: "AcoustoBots是统一平台框架，其中MADDPG负责高层多机器人导航决策，GS-PAT负责底层声学悬浮控制，两者协同实现移动中数据物理化；单机器人模式性能优于双机器人模式，揭示了协同策略仍有改进空间。"
method_debate_generated_at: "2026-09-02T05:39:01.582298Z"
