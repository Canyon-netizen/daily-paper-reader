...
method_pros_cons: {"Distance-constrained skill discovery": {"pros": ["仅用内在奖励与正则奖励", "覆盖更广状态空间", "支持ANYmal零样本目标跟踪"], "cons": ["性能依赖约束阈值与正则设计", "仅验证四足平台与有限任务", "缺少任务奖励或限制任务最优性"]}, "Skill matching objective": {"pros": ["不持续最大化潜转移幅值", "产生更稳定可控的运动技能", "技能幅值可调并扩大覆盖"], "cons": ["范数匹配仍依赖状态表示", "约束超参数需仔细调节", "高维技能组合尚未充分验证"]}, "LSD": {"pros": ["利普希茨约束限定编码关注区", "无需任务奖励即可学习技能", "可在较大状态范围产生行为"], "cons": ["持续最大化潜转移幅值", "易生成过度激进的高速动作", "真实机器人部署稳定性不足"]}, "METRA": {"pros": ["度量感知抽象利于潜空间组织", "支持无监督技能发现与控制", "可复用策略加速下游任务"], "cons": ["高幅值运动倾向限制硬件部署", "行为覆盖与稳定性不及本文方法", "仍需调配合适的奖励权重"]}, "Mutual information maximization": {"pros": ["无需任务奖励即可探索行为", "可学习多样且可复用的技能", "判别器形式能区分不同技能"], "cons": ["KL差异不衡量行为可区分程度", "判别器训练增加实现复杂度", "潜状态轨迹互信息估计困难"]}, "Velocity-following policy": {"pros": ["可在噪声扰动下跟踪速度指令", "训练目标明确且便于工程部署", "可作为标准基线比较技能质量"], "cons": ["常收敛到单一固定步态", "依赖任务奖励与手工设计", "强任务偏置限制行为多样性"]}, "Latent transition-conditioned goal tracking": {"pros": ["无需额外训练即可执行目标跟踪", "可准确到达任意笛卡尔目标", "通过技能幅值调节运动速度"], "cons": ["依赖潜空间到目标坐标的标定", "复杂障碍导航能力尚未验证", "跨平台泛化证据仍有限"]}, "Intrinsic and extrinsic reward scheme": {"pros": ["不依赖任务相关奖励", "正则项促进稳定受控运动", "与内在互信息奖励协同训练"], "cons": ["仍需设计正则奖励与权重", "不保证任务特定的最优性", "奖励尺度失配可能损害学习"]}}
method_comparison: "所提距离约束框架以范数匹配替代LSD和METRA的潜转移最大化，在更广覆盖下获得更稳定、可控的运动，并能直接控制策略进行零样本目标跟踪。传统互信息方法虽降低任务奖励需求，但KL难以刻画行为可区分度；速度跟踪策略稳定直接，却易收敛为单一技能并缺乏行为多样性。"
method_debate_generated_at: "2026-09-02T16:11:37.324205Z"
