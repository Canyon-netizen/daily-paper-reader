...
wiki_compiled: true
wiki_compiled_at: '2026-08-04T01:19:58+00:00'
concepts:
...
method_pros_cons: {"VLM-AR3L": {"pros": ["结合绝对与相对奖励，提升训练稳定性", "在RingWorld等环形任务中优于纯绝对奖励", "跨经典控制、操作和Minecraft等多领域有效", "无需微调VLM或访问特权状态信息"], "cons": ["依赖VLM生成偏好标签，存在标注噪声", "双奖励模型增加架构与训练复杂度", "需额外VLM推理开销进行偏好标注", "在极模糊任务中VLM偏好可能不可靠"]}, "Absolute Reward Model": {"pros": ["为每个状态提供标量评估，简单直观", "与自然语言任务目标对齐，语义清晰", "可与现有RL算法直接集成", "适用于有明确全局状态排序的任务"], "cons": ["训练中跨步骤不一致，信号波动大", "在环形或无序任务中无法定义全局排序", "需持续VLM推理以应对新状态", "长视野任务优化困难"]}, "Relative Reward Model": {"pros": ["通过Siamese网络比较状态对，监督稳定", "累积求和后时间一致性优于绝对奖励", "天然支持环形或无全局排序的任务", "训练后可避免重复VLM推理开销"], "cons": ["仅捕捉局部过渡关系，全局信号有限", "依赖连续状态对的偏好标注质量", "Siamese网络需成对训练数据", "对远距离状态关系建模能力不足"]}, "RL-VLM-F": {"pros": ["利用生成式VLM生成成对偏好，反馈细粒度", "在视觉多样环境中具有较好鲁棒性", "可从人工或VLM反馈中学习奖励", "无需手工设计奖励函数"], "cons": ["仅依赖绝对奖励，训练中信号不稳定", "长视野或环形任务中表现受限", "持续VLM推理带来高计算开销", "对VLM偏好质量高度敏感"]}, "CLIP-based Reward Learning": {"pros": ["基于相似度计算奖励，简单可扩展", "无需任务特定训练即可跨域使用", "可直接利用预训练视觉-语言对齐", "计算高效，适合快速原型"], "cons": ["跨不同领域需任务特定微调", "相似度分数粒度有限，难以精细区分", "在视觉复杂环境中区分能力下降", "无法处理需深层语义理解的任务"]}, "LLM-based Reward Learning": {"pros": ["可生成结构化代码定义奖励逻辑", "支持文本/程序化环境中的灵活奖励", "能处理语言条件任务的复杂规范", "可输出标量奖励信号"], "cons": ["依赖特权状态访问，限制视觉域应用", "难以用代码描述开放世界抽象任务", "在非完全可观测环境中适用性差", "无法直接处理高维视觉观察"]}}
method_comparison: "VLM-AR3L通过融合绝对奖励的稳定性与相对奖励的鲁棒性，在环形任务和长视野场景中优于纯绝对奖励方法RL-VLM-F及基于相似度的CLIP方法，同时避免LLM方法对特权状态的依赖。"
method_debate_generated_at: "2026-09-02T05:33:50.294117Z"
...
在偏好建模层面，论文基于 Bradley-Terry 模型将偏好学习形式化。对于观测对 $(\sigma_i, \sigma_j)$，偏好概率表示为：

$$P[\sigma_i \succ \sigma_j] = \frac{\exp(r(s_i))}{\sum_{k \in \{i,j\}} \exp(r(s_k))}$$

奖励模型通过最小化交叉熵损失训练，使预测的偏好概率与 VLM 生成的标签对齐。

绝对奖励模型的结构相对直接：$R_{abs}^\psi(s_t) = F_{abs}^\psi(s_t) = r_\psi(s_t)$，直接为每个状态输出一个标量值。

相对奖励模型采用孪生网络架构，学习比较函数 $F_{rel}^\phi(s_i, s_j) = P_\phi[\sigma_i \succ \sigma_j]$。为避免连续帧差异过小，论文引入时间偏移 $k \geq 1$，比较 $s_t$ 与 $s_{t-k}$ 而非 $s_t$ 与 $s_{t-1}$。更重要的是，设计了对称置信校验机制来过滤 VLM 误判：

$$R_{rel}^\phi(s_t, s_{t-k}) = \begin{cases} +1, & \text{if } p_t^+ > \tau \text{ and } p_t^- < 1-\tau \\ -1, & \text{if } p_t^- > \tau \text{ and } p_t^+ < 1-\tau \\ 0, & \text{otherwise} \end{cases}$$

其中 $p_t^+ = F_{rel}^\phi(s_t, s_{t-k})$，$p_t^- = F_{rel}^\phi(s_{t-k}, s_t)$，$\tau$ 为置信阈值。只有当双向预测一致且置信度均超过阈值时，才输出确定的奖励值，否则返回中性奖励。这种设计显著降低了错误偏好的影响。

最终，双奖励通过加权融合形成统一信号：

$$R_{dual}^{\psi, \phi}(s_t, s_{t-k}) = \alpha \cdot R_{abs}^\psi(s_t) + (1-\alpha) \cdot R_{rel}^\phi(s_t, s_{t-k})$$

论文发现方法对超参数不敏感，在 $\alpha \in [0.3, 0.7]$、$k$ 多档取值下性能稳定，统一使用 $\alpha=0.5, \tau=0.52, k=16$。
