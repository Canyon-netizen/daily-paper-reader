...
method_pros_cons: {"预测式多智能体RL框架（Proposed）": {"pros": ["结合预测、多智能体与SLA感知", "主动维持SLA稳定而非被动响应", "CTDE支持可扩展在线决策", "仿真中接近oracle性能水平"], "cons": ["依赖预测精度，预测误差会传播", "多智能体协调复杂度较高", "仅基于事件驱动仿真验证", "UAV数量增多时训练成本上升"]}, "MAPPO（集中训练分散执行）": {"pros": ["PPO裁剪机制保证训练稳定", "CTDE架构天然适配多UAV场景", "策略更新可控，收敛性好"], "cons": ["样本效率相对较低", "集中训练阶段需全局状态信息", "超参数敏感，调优开销大"]}, "轻量化移动预测模块": {"pros": ["轻量化设计，计算开销较低", "提前预测拥塞并主动重定位UAV", "避免SLA违规的发生"], "cons": ["预测精度依赖历史轨迹质量", "预测误差会级联影响决策", "额外开销增加系统复杂度"]}, "SLA感知奖励函数": {"pros": ["显式惩罚违规概率与持续时间", "跨HRLLC/eMBB/mMTC切片统一优化", "兼顾能耗与SLA稳定性"], "cons": ["奖励塑形复杂，调参困难", "违规度量定义依赖具体场景", "多目标权衡可能顾此失彼"]}, "Oracle基线": {"pros": ["提供性能上界参考标准", "验证所提方法的逼近程度"], "cons": ["依赖完美未来信息，不可实现", "仅作为理论性能上限参考"]}, "现有反应式RL基线（[5][6][10][11]）": {"pros": ["针对单一目标优化有效果", "实现相对简单，易于部署"], "cons": ["多为反应式，无预测能力", "仅优化传统QoS而非SLA", "缺乏切片级协调机制"]}, "两时间尺度自调整切片RL（[12]）": {"pros": ["结合切片与学习控制", "适应网络动态变化"], "cons": ["缺乏显式SLA违规建模", "未联合轨迹与卸载优化", "两时间尺度耦合设计复杂"]}}
method_comparison: "本文提出的预测式多智能体RL框架通过融合用户移动预测、SLA感知奖励设计与MAPPO的CTDE训练，在SLA稳定性上显著优于现有反应式RL基线和两时间尺度切片方法，同时在能耗与时延上保持竞争力，并随预测精度提升趋近oracle性能。"
method_debate_generated_at: "2026-09-02T15:45:48.164820Z"
