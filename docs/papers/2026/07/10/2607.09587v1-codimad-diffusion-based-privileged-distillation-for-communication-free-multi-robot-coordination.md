---
title: 'CoDiMAD: Diffusion-Based Privileged Distillation for Communication-Free Multi-Robot
  Coordination'
...
figures_json: '[{"url": "assets/figures/arxiv/2607.09587v1/fig-001.webp", "caption":
  "", "page": 0, "index": 1, "width": 1432, "height": 593}, {"url": "assets/figures/arxiv/2607.09587v1/fig-002.webp",
  "caption": "", "page": 0, "index": 2, "width": 763, "height": 294}, {"url": "assets/figures/arxiv/2607.09587v1/fig-003.webp",
  "caption": "", "page": 0, "index": 3, "width": 2685, "height": 1062}, {"url": "assets/figures/arxiv/2607.09587v1/fig-004.webp",
  "caption": "", "page": 0, "index": 4, "width": 705, "height": 753}]'
...
wiki_compiled: true
wiki_compiled_at: '2026-08-04T01:46:17+00:00'
concepts:
- slug: privileged-policy-distillation
  display_name: Privileged Policy Distillation
  category: method
  novelty: 0.3
  centrality: 0.95
- slug: denoising-diffusion-probabilistic-model
  display_name: Denoising Diffusion Probabilistic Model
  category: method
  novelty: 0.2
  centrality: 0.95
...
- slug: mappo
  ...
  novelty: 0.0
  centrality: 0.6
- slug: multi-modal-action-distribution
  display_name: Multi-Modal Action Distribution
  category: problem
  novelty: 0.4
  centrality: 0.85
- slug: decentralized-multi-robot-coordination
  display_name: Decentralized Multi-Robot Coordination
  category: problem
  novelty: 0.1
  centrality: 0.9
...
method_pros_cons: {"CoDiMAD": {"pros": ["首个将扩散生成策略集成到多机器人特权蒸馏的框架", "通过条件扩散建模解决多模态模糊性问题", "结合DDIM实现少步采样，支持实时机载部署", "在三个协作任务上持续优于直接局部MARL和确定性蒸馏基线"], "cons": ["依赖MAPPO预训练oracle，训练流程分三阶段较复杂", "扩散模型推理延迟仍存在，需额外引入DDIM缓解", "理论分析聚焦模态平均现象，泛化性边界未充分讨论"]}, "MAPPO": {"pros": ["利用全局状态信息简化学习问题，oracle训练更高效", "成熟的CTDE算法，提供稳定集中式批评家指导", "在多智能体协作任务中性能已被广泛验证"], "cons": ["依赖全局状态访问，部署时不可用", "集中式训练架构需要共享信息和通信", "直接从局部观察端到端训练收敛慢且易陷入次优策略"]}, "DDPM（条件去噪扩散概率模型）": {"pros": ["可建模完整条件动作分布而非点估计，避免模态平均", "通过反向去噪过程从连贯协调模式中采样决断性动作", "理论上能近似条件oracle动作分布"], "cons": ["推理延迟较高，需要多步迭代去噪", "训练计算成本相对较高", "采样过程随机性可能引入额外不确定性"]}, "DDIM（去噪扩散隐式模型）": {"pros": ["支持少步加速采样，降低推理延迟", "保持与DDPM相似的生成质量", "使扩散策略可部署到实时机载系统"], "cons": ["少步采样可能略微牺牲生成多样性", "加速效果依赖步数选择，步数过少影响性能"]}, "CTDE范式": {"pros": ["训练时利用全局信息，部署时仅需局部观察", "兼顾训练效率与部署可扩展性", "广泛应用于多智能体强化学习算法"], "cons": ["仅靠集中式批评家难以处理高度部分可观测场景", "actor策略仍受局部观察限制，在无通信下协调困难"]}, "确定性蒸馏（基于MSE的行为克隆）": {"pros": ["实现简单，训练成本低", "在单机器人特权蒸馏中已验证有效（如视觉驾驶、本体感觉运动）"], "cons": ["MSE目标收敛到条件均值，跨越不同模态导致无效动作", "无法表示多模态条件动作分布，产生犹豫或退化协调", "在多智能体部分可观测下出现模态平均失效"]}, "直接局部MARL": {"pros": ["无需特权信息或全局状态，部署简单", "避免蒸馏过程的额外训练阶段"], "cons": ["在部分可观测下收敛慢，易陷入次优协调策略", "无法利用全局信息缓解观测歧义性", "在通信受限环境下性能严重受限"]}, "学习通信信道方法": {"pros": ["可在执行阶段交换任务相关信息，缓解部分可观测性", "在通信可靠时表现优异"], "cons": ["依赖可靠通信假设，在水下声学延迟等场景不可用", "带宽受限或通信中断时性能显著下降"]}}
method_comparison: "本文对比了直接局部MARL、确定性回归蒸馏和学习通信信道等基线方法，指出它们在部分可观测、无通信条件下分别面临收敛慢、模态平均和通信依赖等问题，而CoDiMAD通过MAPPO训练特权oracle并结合条件DDPM与DDIM蒸馏，在保持去中心化部署的同时有效恢复多模态协调分布。"
method_debate_generated_at: "2026-09-02T05:45:39.572410Z"
...
**第一阶段：训练特权 Oracle**。使用 MAPPO 在双流输入下训练 oracle 策略 $\pi_O$，使其同时访问局部传感器信息和全局鸟瞰图（BEV）。局部流包含 LiDAR 占用栅格和自身速度场等共享通道；全局流则包含自车和队友位置热力图、任务状态等特权信息。这种双流设计让 oracle 能够准确区分看似相似但本质不同的全局构型。

**第二阶段：构建离线数据集**。冻结训练好的 oracle，让其在学生视角下执行 rollout，只保留成功且无碰撞的回合。数据集中的每条记录包含学生获得的局部观测 $o_t^i$ 和 oracle 对应的动作 $a_t^i$。数据筛选机制确保监督信号的质量——失败或存在碰撞的轨迹不参与蒸馏过程。

**第三阶段：扩散式行为克隆**。这是 CoDiMAD 的核心创新。与传统回归式蒸馏不同，学生策略被参数化为 [[条件去噪扩散概率模型]]（Conditional DDPM），通过学习去噪网络 $\hat{\epsilon}_\theta$ 来近似 oracle 条件动作分布。
...
其中 $c$ 是由 CNN 和 GRU 编码器从局部观测提取的条件嵌入。推理时使用 [[DDIM]] 加速采样，将 200 步去噪压缩至 20 步，接近实时控制的时间预算。

这种设计在理论上具有清晰的意义：扩散模型在无限数据和模型容量下等价于估计条件分数函数 $\nabla_{a^{(k)}} \log q_k(a^{(k)} \mid c)$，其采样过程能够完整还原 oracle 的多模态条件分布，而非坍缩为均值。
...
然而，这项工作也存在明显的局限性。首先，实验仅在 $N=3$ 同构机器人设定下验证，随 agent 数量增加协调空间的复杂度会急剧上升，异构多智能体的适应性尚未探索。其次，离线蒸馏固有的分布漂移风险意味着学生在遇到 oracle 训练分布之外的状态时可能失效，论文提到可引入扩散兼容的在线微调方法（如 IDQL）作为后续方向。此外，仿真到真实环境的迁移仍是一项开放挑战，水下和水面的传感器噪声、动力学建模差异都可能破坏蒸馏策略的有效性。
