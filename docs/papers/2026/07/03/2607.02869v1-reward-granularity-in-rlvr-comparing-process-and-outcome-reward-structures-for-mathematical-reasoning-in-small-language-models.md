...
title_zh: RLVR 中的奖励粒度:比较小语言模型数学推理的过程奖励与结果奖励结构
...
figures_json: '[{"url": "assets/figures/arxiv/2607.02869v1/fig-001.webp", "caption":
  "", "page": 0, "index": 1, "width": 1991, "height": 590}, {"url": "assets/figures/arxiv/2607.02869v1/fig-002.webp",
  "caption": "", "page": 0, "index": 2, "width": 1760, "height": 900}, {"url": "assets/figures/arxiv/2607.02869v1/fig-003.webp",
  "caption": "", "page": 0, "index": 3, "width": 2913, "height": 1473}, {"url": "assets/figures/arxiv/2607.02869v1/fig-004.webp",
  "caption": "", "page": 0, "index": 4, "width": 3600, "height": 2100}]'
motivation: 系统研究RLVR奖励结构 使用GRPO。
method: 方法与实现细节请参考摘要与正文。
result: 结果与对比结论请参考摘要与正文。
conclusion: 总体而言，该工作在所述任务上展示了有效性，并提供了可复用的思路或工具。
wiki_compiled: true
wiki_compiled_at: '2026-08-04T01:25:49+00:00'
concepts:
- slug: rlvr
...
method_pros_cons: {"No-RL Baseline": {"pros": ["作为对照基准无需强化学习训练", "避免RL训练的不稳定性"], "cons": ["缺乏奖励信号推理能力有限", "无法利用过程级反馈提升质量"]}, "Process-Only Reward": {"pros": ["测试准确率达63.73%为最佳配置", "推理链步骤有效性更高", "偏离真实链长度的程度更低"], "cons": ["可能产生过长推理链", "小模型在密集反馈下训练不稳定"]}, "Outcome-Only Reward": {"pros": ["仅需验证最终答案实现简单", "训练信号稀疏更稳定易收敛"], "cons": ["准确率仅53.73%低于过程奖励约10个百分点", "易导致推理错误但答案巧合正确"]}, "Hybrid Reward (λ=0.9)": {"pros": ["以过程为主保留高步级有效性", "兼具过程监督的高准确率优势"], "cons": ["仍受过程奖励不稳定性的影响", "可能继承过程方法的链长过长问题"]}, "Hybrid Reward (λ=0.5)": {"pros": ["平衡过程与结果信号训练较稳定", "避免单一奖励类型的极端偏差"], "cons": ["准确率低于纯过程奖励方案", "优化信号分心可能削弱双方优势"]}, "Hybrid Reward (λ=0.1)": {"pros": ["保留过程信号以维持步级引导", "保持结果验证的全局约束"], "cons": ["表现劣于纯结果奖励存在异常冲突", "过程与结果信号矛盾导致优化冲突"]}, "RLVR (GRPO) Framework": {"pros": ["基于可验证奖励无需人类偏好标注", "GRPO适合小模型群体相对优化", "在GSM8K上数学推理可自动验证"], "cons": ["奖励粒度设计对效果影响显著", "小模型对稀疏反馈自纠能力不足"]}, "Chain-of-Thought Prompting (CoT)": {"pros": ["引导显式中间步骤提升可解释性", "通常能改善最终答案准确率"], "cons": ["小模型生成步骤易出现幻觉和逻辑不一致"]}, "RLHF": {"pros": ["基于人类偏好对齐改善答案准确率", "广泛验证的成熟对齐框架"], "cons": ["仅结果级奖励无过程引导", "易被模型利用捷径和表面模式"]}}
method_comparison: "论文在RLVR框架下系统比较五种奖励粒度配置，发现过程奖励准确率显著优于结果奖励约10个百分点，但λ=0.1的混合配置出现反常劣化现象，说明奖励粒度是RLVR的一阶设计决策，过程级监督在小模型上能同时提升准确率与推理链忠实度。"
method_debate_generated_at: "2026-09-01T12:22:53.331267Z"
...
关于奖励函数的设计，论文定义了三种类型的奖励机制。结果奖励（Outcome Reward）采用最朴素的方式：提取模型生成文本中的最终答案（支持 `\boxed{answer}`、`#### answer` 等常见格式），与真实答案进行比对，在 $10^{-5}$ 的数值容差范围内匹配即给予 1 分，否则为 0 分。这种二元奖励信号极为稀疏——模型只有在整道题答对时才能获得正向反馈。

过程奖励（Process Reward）则将监督信号细化到推理的每一步。具体而言，论文计算生成链中与真实解法中间步骤对齐的比例：

$$R_{\text{process}} = \frac{\text{correct steps}}{\text{total steps}}$$

每一步的判断标准是：该步骤的计算结果是否与 ground-truth 中对应位置的计算值一致（同样采用 $10^{-5}$ 容差）。为了防止模型通过生成大量冗余步骤来"刷分"，论文还设置了一条规则：当生成链长度超过真实解法步数的 1.5 倍时，施加额外的惩罚。

混合奖励（Hybrid Reward）则将两者加权组合：

$$R = \lambda \cdot R_{\text{process}} + (1 - \lambda) \cdot R_{\text{outcome}}$$

实验设置了 $\lambda \in \{0.9, 0.5, 0.1\}$ 三种混合权重，分别代表重过程、平衡、轻过程三种配置。

在训练流程上，模型通过 vLLM 推理引擎生成多个 rollouts（每个样本生成 5 条），奖励函数对每条 rollout 进行评估后，使用 GRPO 算法更新策略。所有实验共享同一组超参数（batch size 64、学习率 $1 \times 10^{-6}$、KL 系数 0.001、temperature 0.6 等），唯一变化的变量就是奖励函数设计，这样的设置最大化了实验结果的可归因性。
