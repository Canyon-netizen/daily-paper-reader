...
method_pros_cons: {"Anticipatory DRL (Predictive Formulation)": {"pros": ["仿真中实现约9倍误差降低，从2.73°降至0.31°", "融合了MPC的前瞻性与DRL的轻量推理优势", "无需每步求解优化问题，计算开销低", "提供多种预测配置以适配不同场景"], "cons": ["零样本迁移到真实硬件存在明显sim-to-real差距", "最复杂配置(N=4, Δt=0.5s)在仿真最优但实际表现并非最佳", "仅在1-DoF直升机测试台验证，泛化性待验证", "依赖精确的未来参考轨迹信息，实际中可能难获取"]}, "Purely Reactive DRL (Baseline)": {"pros": ["实现简单，状态空间最小，计算需求低", "作为对比基准明确揭示预测信息的价值", "无需未来参考轨迹信息，适用性广"], "cons": ["因纯反应式控制导致明显滞后和超调", "跟踪误差高达2.73°，性能最差", "无法预知轨迹变化，跟踪动态信号能力不足"]}, "Model Predictive Control (MPC)": {"pros": ["系统性地融入未来参考轨迹，具有真正前瞻性", "基于模型的优化提供理论性能保证", "在工业控制中成熟可靠"], "cons": ["每步需求解优化问题，计算开销高", "实时工业应用中受限，推理延迟较大", "依赖精确的系统数学模型，建模困难"]}, "Proximal Policy Optimization (PPO)": {"pros": ["具有理论策略改进保证，连续动作空间中稳定性好", "通过Stable Baselines3易于实现和复现", "支持多种状态空间配置的灵活训练"], "cons": ["对超参数和奖励设计敏感，训练成本较高", "依赖仿真环境训练，面临sim-to-real迁移挑战", "每个配置需10次独立训练，计算资源消耗大"]}}
method_comparison: "论文提出Anticipatory DRL在仿真中显著优于纯反应式DRL基线（误差降低9倍），但在硬件迁移时简单单点远距前瞻配置(N=1, Δt=1.0s)反而匹配了最复杂配置的实际表现（均约1.11°），表明细粒度预测数据并非物理迁移所必需；相比MPC，Anticipatory DRL以极低的推理计算开销获得了类似的前瞻控制能力。"
method_debate_generated_at: "2026-09-01T12:23:48.711178Z"
