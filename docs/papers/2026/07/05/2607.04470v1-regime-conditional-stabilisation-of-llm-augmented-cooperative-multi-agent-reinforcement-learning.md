...
method_pros_cons: {"Potential-Based Reward Shaping (PBRS)": {"pros": ["理论保证最优策略不变", "形式简洁F=γΦ(s')-Φ(s)", "经典成熟的塑形框架"], "cons": ["要求势函数平稳不变", "动态更新时打破理论保证", "off-policy replay下不一致最严重"]}, "Phase-Based Freeze Schedule": {"pros": ["阶段内严格保持平稳性", "实现简单无需调参", "恢复PBRS理论保证"], "cons": ["阶段切换时仍有非平稳跳变", ["需预先设计阶段划分", "可能延迟新权重的生效"]]}, "Exponential Moving Average (EMA) Smoothing": {"pros": ["Simple Spread成功率86.7% vs 15.2%", "+12.3pp提升且p<0.01", "α=0.2经消融实验选定", "有效限制每episode权重漂移"], "cons": ["仍非严格平稳", "引入超参数α需调优", "可能延迟权重更新响应"]}, "QMIX": {"pros": ["主流值分解MARL基线", "样本高效off-policy学习", "CTDE框架成熟"], "cons": ["依赖单调混合器假设", "replay buffer对非平稳敏感", "值分解结构增加复杂度"]}, "VDN": {"pros": ["简单加性值分解", "验证分类法跨架构适用性", "作为QMIX的对比基线"], "cons": ["仅作为探索性扩展", "表达能力弱于QMIX", "未做完整实验验证"]}, "Naive Dynamic LLM Shaping": {"pros": ["灵活响应人类目标变化", "无需重启训练即可更新", "贴近LLM自然部署模式"], "cons": ["污染replay buffer存储的奖励标签", "Simple Spread上崩溃至15.2%", "违反PBRS平稳性假设", "在augmentative regime导致灾难性失败"]}}
method_comparison: "论文提出EMA和平稳冻结两种互补的稳定化策略，EMA在augmentative regime显著优于naive动态更新（86.7% vs 15.2%），而Phase-Based Freeze Schedule通过强制阶段平稳性从理论上恢复PBRS保证，两者分别从连续平滑和离散阶段两个角度控制非平稳性。"
method_debate_generated_at: "2026-09-01T12:27:56.467969Z"
