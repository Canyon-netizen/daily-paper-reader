...
method_pros_cons: {"MRCL": {"pros": ["基于2025年后多模态数据集，减少预训练数据污染", "任务多样化，提供可靠的遗忘评估测试平台", "同时衡量稳定性与塑性（Pareto前沿）", "支持多模型规模评估（2B/4B/8B）"], "cons": ["仅覆盖多模态推理场景，不适用于纯文本任务", "依赖最新数据集，长期可复现性存疑"]}, "CPO": {"pros": ["理论基于先验任务行为KL目标，有数学保证", "无需回放数据，保护数据隐私与效率", "在Qwen3-VL-8B上遗忘减少13.7%，预训练能力提升7.0%", "参数移动正则化仅作用于高变化子集，计算开销小"], "cons": ["需累积训练过程中的参数移动信息，实现较复杂", "局部松弛的KL近似在大规模模型上精度待验证"]}, "GRPO": {"pros": ["KL正则化可防止当前任务过度优化", "在多种推理任务上表现稳定"], "cons": ["KL约束基于当前任务数据，无法有效缓解遗忘", "正则化可能降低模型学习新任务的可塑性"]}, "GSPO": {"pros": ["移除了KL项，提升学习容量与训练效率"], "cons": ["移除KL约束后灾难性遗忘更为严重", "缺少对先验任务行为漂移的约束机制"]}, "DAPO": {"pros": ["增强了RL训练的探索能力与稳定性"], "cons": ["移除KL项，无法抑制参数对旧任务的漂移", "在持续学习场景下遗忘问题未被解决"]}, "SFT（全参数微调）": {"pros": ["实现简单，工具链成熟，易于部署"], "cons": ["灾难性遗忘严重，文中图1显示CAD降幅最大", "所有参数更新，难以保留先验能力"]}, "LoRA": {"pros": ["参数高效，仅更新低秩适配器，节省显存"], "cons": ["在持续学习场景下仍存在明显遗忘", "低秩约束可能限制新任务的表达能力"]}, "EWC": {"pros": ["经典二阶近似方法，理论清晰可解释"], "cons": ["需额外梯度计算与Fisher估计，显存开销大", "不适用于大规模VLM后训练场景"]}}
method_comparison: "论文对比了RL类方法（GRPO/GSPO/DAPO）与SFT类方法（全参数SFT/LoRA），证明现有方法均存在严重遗忘，并提出基于先验任务KL近似的CPO框架，在无需回放的前提下显著优于所有基线。"
method_debate_generated_at: "2026-09-01T12:27:05.596100Z"
