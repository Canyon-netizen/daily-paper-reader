...
method_pros_cons: {"Mosaic": {"pros": ["执行速度提升27-32%，显著降低延迟", "LLM调用减少30-33%，降低推理开销", "任务成功率提升4-10个百分点", "同时解决状态追踪与协调两大瓶颈"], "cons": ["依赖ILP求解器，增加额外计算开销", "需在AI2-THOR等仿真环境中验证", "对LLM候选动作质量存在依赖"]}, "Agent-centric Semantic Memory": {"pros": ["以相对坐标存储，轻量且可扩展", "支持跨智能体坐标变换与几何推理", "物体离开视野后可从轨迹推断位置", "兼顾空间精度与计算效率"], "cons": ["依赖运动历史准确性，累积误差风险", "物体被遮挡时需额外推断机制", "相对坐标需持续维护与更新"]}, "Action Selection via ILP": {"pros": ["每步分配动作，支持细粒度协同", "强制物理可行性约束，减少失败动作", "解决动作级别冲突与冗余问题", "适应动态环境变化"], "cons": ["ILP求解带来额外计算开销", "受限于LLM生成的候选动作集合", "需为每步重新构建优化问题"]}, "Temporal Memory (Nayak等; Yao等; Lin等)": {"pros": ["实现轻量，易于与LLM集成", "无需额外几何计算开销"], "cons": ["迫使LLM从文本推断空间关系，易出错", "无法显式编码几何信息", "导致不可达目标导航等空间推理失误"]}, "Spatial/Spatio-temporal Memory (Fang等; Hu等; Mao等; He等)": {"pros": ["显式编码几何结构，空间精度高", "支持精确的空间关系推理"], "cons": ["计算成本高，扩展性受限", "内存开销随环境规模增长", "在多智能体场景下效率低下"]}, "Coarse-grained Target-Agent Allocation (Choe等; Brawer等; Obata等; Wu等)": {"pros": ["实现简单，易于部署", "可处理基本的任务级冲突"], "cons": ["固定职责分配，无法适应环境变化", "无法解决动作级别的细粒度决策", "无法避免动作前置条件违反"]}}
method_comparison: "Mosaic通过相对坐标语义记忆与ILP动作分配，分别针对现有时序记忆（轻量但空间推理差）和空时记忆（精确但计算昂贵）的权衡，以及粗粒度目标分配（仅任务级协调）的局限，实现了轻量状态追踪与细粒度协同的统一。"
method_debate_generated_at: "2026-09-02T05:48:21.757895Z"
