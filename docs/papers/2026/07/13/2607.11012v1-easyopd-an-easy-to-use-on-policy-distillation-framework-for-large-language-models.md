...
method_pros_cons: {"EasyOPD": {"pros": ["三层架构解耦用户/方法/执行层", "通过YAML配置统一多种OPD方法", "基于verl分布式后端保证可扩展性", "提供明确扩展点支持方法本地定制"], "cons": ["依赖verl后端生态限制独立性", "仅实例化代表性方法非穷尽覆盖", "方法间性能差异仍依赖任务特性", "实验规模有限未充分验证大规模场景"]}, "Conventional Offline Distillation": {"pros": ["实现简单无需在线交互", "训练过程稳定可预测", "可复用已有教师生成数据"], "cons": ["固定数据无法覆盖学生演变状态", "存在分布不匹配影响泛化", "推理时学生遭遇离线未覆盖上下文"]}, "On-Policy Distillation (OPD)": {"pros": ["通过学生rollout收集监督解决分布不匹配", "覆盖多种监督粒度适应不同场景", "可整合教师推理与评估信号"], "cons": ["现有实现碎片化难以复现扩展", "不同方法在监督形式上差异大", "需额外开销支持教师推理与对齐"]}, "Cross-tokenizer OPD": {"pros": ["支持异构分词器间蒸馏扩展灵活性", "放宽教师-学生模型组合限制"], "cons": ["需额外分词器对齐步骤增加复杂度", "对齐过程可能引入误差影响监督质量"]}, "On-policy Self-Distillation (OPSD)": {"pros": ["无需不同分词器的外部教师", "学生自主迭代提升路径简洁", "避免教师-学生分词器不兼容问题"], "cons": ["缺乏外部强信号可能限制提升幅度", "依赖自身策略质量存在天花板效应"]}, "Step-wise OPD": {"pros": ["提供细粒度逐步监督信号", "适合多轮智能体与工具使用场景", "可结合过程奖励增强学习"], "cons": ["监督粒度细导致实现复杂度高", "对评估器或逐步奖励依赖较强"]}, "TRL": {"pros": ["提供训练器为中心的蒸馏API生态成熟", "社区支持广泛易于上手"], "cons": ["缺乏方法本地扩展边界机制", "非统一的OPD专用后端", "对异构监督接口支持有限"]}, "verl": {"pros": ["支持可扩展分布式在线策略执行", "提供完整rollout与训练基础设施"], "cons": ["非专门为蒸馏设计需额外抽象层", "对方法本地监督扩展支持不足"]}, "slime": {"pros": ["支持可扩展在线rollout生成", "针对LLM训练优化"], "cons": ["非专门蒸馏框架扩展性受限", "缺乏统一方法注册与调度机制"]}, "KDFlow": {"pros": ["提供蒸馏导向的完整工具栈", "支持多种蒸馏工作流"], "cons": ["缺乏方法本地扩展边界设计", "未文档化异构监督接口的集成"]}}
method_comparison: "EasyOPD通过三层架构和扩展点机制统一了传统离线蒸馏、跨分词器OPD、自蒸馏及逐步OPD等碎片化实现，相比TRL、verl、slime和KDFlow等现有框架，核心优势在于提供方法本地扩展边界同时复用verl分布式后端，使多种监督形式的OPD方法可通过同一基础设施执行。"
method_debate_generated_at: "2026-09-02T15:47:19.456647Z"
