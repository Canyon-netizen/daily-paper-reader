...
method_pros_cons: {"COLMAR": {"pros": ["共享策略实现多智能体协调视点分配", "重建感知奖励促进覆盖与避碰", "无需智能体间消息传递即可决策", "在GLEAM和Replica上提升54%精度和49%覆盖"], "cons": ["依赖参数共享，团队规模扩展性待验证", "无智能体通信可能限制复杂场景协作", "仅在室内数据集评估，室外泛化未知"]}, "Frontier/NBV启发式方法": {"pros": ["实用性强、可解释性好", "计算开销低、易于部署", "几何约束满足度高"], "cons": ["手工评分项易产生短视路径", "复杂几何下表现脆弱", "难适应部分可观测和运动约束"]}, "GenNBV": {"pros": ["可学习策略优于固定启发式", "自由空间动作参数化更灵活", "具备一定跨场景泛化能力"], "cons": ["未充分利用密集重建反馈信号", "仅处理单智能体场景，无协调", "奖励信号粗糙，策略稳定性有限"]}, "GLEAM": {"pros": ["复杂室内场景下泛化能力强", "面向主动建图优化", "在多种场景表现稳健"], "cons": ["缺乏多智能体协调机制", "未集成重建驱动的密集奖励", "团队规模增大时易产生冗余观测"]}, "3D高斯泼溅(3DGS)": {"pros": ["高保真光度重建与渲染评估", "训练和渲染效率优于隐式神经表示", "支持快速增量更新"], "cons": ["仅用于评估，不参与策略学习", "存储开销随场景规模增长", "对初始位姿精度敏感"]}, "参数共享PPO": {"pros": ["实现简单高效的共享策略训练", "支持独立分布式动作选择", "策略梯度稳定，适合复杂奖励"], "cons": ["信用分配在共享参数下较模糊", "奖励工程依赖密集重建信号设计", "超参数敏感，训练成本较高"]}}
method_comparison: "COLMAR通过共享策略与重建感知奖励实现多智能体协调视点分配，相比启发式方法泛化性更强，相比GenNBV/GLEAM等单智能体学习方法引入了团队级覆盖与避碰机制，并结合3DGS实现高保真评估。"
method_debate_generated_at: "2026-09-02T15:57:51.173145Z"
