...
method_pros_cons: {"TRACE": {"pros": ["无需评论家或过程奖励模型，纯RL即可训练", "Qwen3-4B在BrowseComp-Plus从7.2提升到35.6", "TD变化在冗余调用上可伸缩，不会虚增信用", "学习曲线更早提升、收敛更快，并可迁移到开放网络"]}, "outcome-only RL": {"pros": ["实现简单，仅依赖终局可验证奖励", "在短程推理任务（如数学、编程）上可靠且稳定"], "cons": ["长程轨迹下奖励稀疏、梯度方差高", "对有用中间动作和最终错误施加相同负优势", "无法区分贡献大小，浪费有效探索信号"]}, "过程监督/PRM": {"pros": ["可提供比终局奖励更细粒度的逐步反馈", "在已有逐步标注的场景中可改善信用分配"], "cons": ["需逐步标签或额外训练的强LLM裁判，成本高", "训练好的过程奖励模型评分可能偏离最终正确性", "依赖大量过程标注数据，难以泛化到新领域"]}}
method_comparison: "TRACE在保持终局验证奖励锚点的同时，利用冻结参考模型的金答案对数概率提供密集信用，避免了过程奖励模型对标签和裁判模型的依赖；相比仅用终局奖励的稀疏高方差方法以及需要逐步标注的过程监督方法，TRACE以更低监督成本在长程搜索任务上取得了显著且可迁移的提升。"
method_debate_generated_at: "2026-09-02T15:58:17.293380Z"
