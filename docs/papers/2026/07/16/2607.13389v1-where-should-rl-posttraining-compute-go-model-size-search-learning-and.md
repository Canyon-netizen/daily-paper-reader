...
这篇论文系统研究了强化学习后训练中一个被长期忽视的问题：在固定的算力预算下，到底应该把钱花在更大的模型、更多的训练步数、更多的样本搜索，还是更强的奖励信号上？研究者提出了一个将 GRPO 后训练的算力分解为 rollout、学习和反馈三部分的核算框架，在 Qwen2.5 系列模型上的实验揭示了「条件性分配前沿」现象——最优算力分配方案会随着模型规模、预算高低、奖励系统类型和评估任务的不同而变化。这些发现意味着，简单报告一个总 FLOPs 数字远远不够，论文建议 RL 后训练研究应该透明地交代算力在模型、搜索、学习与反馈之间的具体分配。
...
要回答「钱该怎么花」这个问题，首先得搞清楚「钱花在了哪里」。研究者针对 GRPO（Group Relative Policy Optimization）后训练提出了一个 FLOP 核算框架，将总算力消耗明确分解为三个核心组成部分：
