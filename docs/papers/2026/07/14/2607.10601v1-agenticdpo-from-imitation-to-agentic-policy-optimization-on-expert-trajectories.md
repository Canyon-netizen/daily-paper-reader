...
method_pros_cons: {"SFT (Supervised Fine-Tuning)": {"pros": ["简单稳定，实现成本低", "无需在线环境交互", "在多个基准上作为可靠基线"], "cons": ["仅模仿token序列，不优化决策", "无法针对错误动作提供反馈", "在τ-bench上9B模型仅21.7%成功率"]}, "Agentic-DPO": {"pros": ["离线优化，无需在线环境rollout", "无需奖励模型或全轨迹探索", "τ-bench上将9B模型从21.7%提升至41.4%", "匹配同骨干在线GRPO性能且成本更低"], "cons": ["仍需单步动作采样", "依赖专家轨迹质量", "偏好对构造质量影响最终效果"]}, "Policy-Preserving Augmentation (PPA)": {"pros": ["保持专家策略不变，仅变换schema", "消除策略与schema的混淆", "使偏好方向对齐潜在策略"], "cons": ["增加实现复杂度", "需维护多种schema渲染", "依赖schema多样性"]}, "GRPO (在线强化学习基线)": {"pros": ["在线强化学习性能强劲", "在τ-bench上达40.0%成功率"], "cons": ["需要在线环境交互，成本高", "训练流程复杂，计算开销大", "依赖环境可执行性"]}, "Step-DPO / HPL / PivotRL / Agent-RLVR (现有偏好与RL方法)": {"pros": ["利用轨迹内决策结构提升训练", "在多种场景已验证有效"], "cons": ["常需代价高的步骤选择或人工标注", "依赖可执行环境进行rollout", "训练流程复杂且成本较高"]}, "ATLaS / SWE-Lego (SFT改进方法)": {"pros": ["通过筛选有价值的动作步改进SFT", "实现相对简单"], "cons": ["依赖启发式规则或外部标注模型", "仍未解决错误动作的反馈问题"]}}
method_comparison: "Agentic-DPO通过将专家轨迹转为状态级偏好信号，在无需在线环境交互和奖励模型的情况下，匹配甚至超越在线GRPO等强化学习方法，显著优于仅做token模仿的SFT基线，同时其PPA组件解决了偏好学习中策略与schema混淆的关键问题。"
method_debate_generated_at: "2026-09-02T05:52:20.972432Z"
