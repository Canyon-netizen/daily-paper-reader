...
method_pros_cons: {"OPSD (On-Policy Self-Distillation)": {"pros": ["避免off-policy模仿的曝光偏差", "避免RL稀疏奖励瓶颈", "在通用对齐任务中效果显著"], "cons": ["复杂推理任务中反而降低下游性能", "导致思维密度从9.8骤降至7.9 ET/1k", "在学生高熵决策分叉处激进压制认知token"]}, "Thinking Collapse（思维崩溃诊断）": {"pros": ["用ET per 1k提供长度不变的量化度量", "多尺度证据链定位失败边界", "揭示高分歧区域即高抑制风险区"], "cons": ["仅诊断现象，未提供解决机制", "依赖top-20%熵遮蔽才能部分恢复密度", "无法同时恢复下游任务准确率"]}, "Entropy-based Gradient Masking（熵梯度遮蔽）": {"pros": ["可定位崩溃发生的高熵决策分叉空间边界", "遮蔽top-20%高熵token即可恢复ET至9.8", "为AD-OPSD设计提供关键启发"], "cons": ["二元硬遮蔽粒度过粗", "恢复思维密度后准确率仍偏低", "存在纠错能力与思维保留的权衡"]}, "Token-level Target Analysis（token级目标分析）": {"pros": ["揭示Student-ET→Teacher-Non-ET的强制压制", "提供初始化的细粒度诊断证据", "可识别认知token被替换的具体模式"], "cons": ["仅展示初始化静态状态，缺乏动态视角", "分析维度单一，需配合其他诊断使用"]}, "Pointwise Divergence Mapping（逐点分歧映射）": {"pros": ["识别高抑制风险区域，支持实时干预", "证明认知步骤密集分布于高分歧区", "为软门控机制设计提供理论支撑"], "cons": ["依赖学生-教师分歧阈值设定，敏感性高", "需要额外计算开销进行逐token分歧估计"]}, "AD-OPSD (Adaptive Dual-Perspective OPSD)": {"pros": ["软门控机制保留纠错能力的同时保护思维", "在多模型多数据集上平均准确率提升+4.1%", "泛化至非思维对齐等多种后训练范式", "连续非对称门控比二元遮蔽更精细"], "cons": ["引入冻结基模型参考先验，增加内存开销", "高熵沙箱范围与门控阈值需额外调参", "实验仅在数学基准验证，其他领域待考察"]}}
method_comparison: "本文以OPSD为基线，通过熵梯度遮蔽、token级目标分析和逐点分歧映射三个诊断工具揭示思维崩溃机制，并提出AD-OPSD用非对称软门控在保留纠错能力的同时恢复思维密度，相比标准OPSD在数学基准上取得+4.1%绝对提升。"
method_debate_generated_at: "2026-09-02T05:53:08.174617Z"
