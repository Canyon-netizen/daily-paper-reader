...
method_pros_cons: {"Deterministic Policy Gradient for Extended MFC (Proposed)": {"pros": ["避免在随机核上优化，降低无限维复杂度", "状态-动作分布由状态律推导出，策略优化更直接", "相比离散化方法提高计算效率与稳定性", "适用扩展MFC含控制分布依赖的一般情形"], "cons": ["依赖粒子近似，群体规模增大时计算负担高", "确定性策略探索能力有限，需额外加噪声", "理论保证依赖McKean-Vlasov动力学的正则性条件", "优势率函数估计在Wasserstein空间上仍较困难"]}, "Model-Free Sensitivity Formula for McKean-Vlasov Dynamics": {"pros": ["无需模型即可对参数化McKean-Vlasov求梯度", "为策略梯度推导提供理论支撑", "适用于一般非可分系数结构", "结合性能差引理，结构清晰"], "cons": ["依赖状态分布对参数的鲁棒性假设", "推导复杂，实际数值实现门槛较高", "对分布变化敏感，估计方差可能较大"]}, "Local Advantage-Rate Representation": {"pros": ["将梯度分解为动作导数与测度导数项", "比直接从状态律流学习更具信息量", "支持基于martingale的在线时间差分学习", "去除对系数可分结构的限制性假设"], "cons": ["需同时估计测度导数项，增加样本复杂度", "依赖联合状态-动作分布的近似质量", "实现复杂度高于全局优势率方法"]}, "Stochastic Policies for MFC (Existing)": {"pros": ["天然支持探索，策略覆盖更广", "理论发展较早，文献丰富", "适用于一般随机控制问题"], "cons": ["需优化随机核，无穷维问题难以处理", "在扩展MFC中难以处理控制分布依赖", "相比确定性策略计算效率较低"]}, "Discrete-Time MFC RL (Q-learning, MF-REINFORCE)": {"pros": ["动态规划原理成熟，实现简单", "MF-REINFORCE有偏差与方差理论保证", "基于局部策略可高效学习"], "cons": ["依赖时间离散化，步长影响精度", "连续时间下需额外近似处理", "在扩展MFC含控制分布时受限"]}, "Discretization Approach (Continuous-time RL)": {"pros": ["实现相对简单，可复用离散算法", "对模型假设要求较少"], "cons": ["离散误差随步长累积，精度受限", "相比确定性策略方法稳定性较差", "在无限维MFC场景下扩展困难"]}}
method_comparison: "本文提出的连续时间扩展MFC确定性策略梯度框架，相比随机策略方法避免了无穷维随机核优化，相比离散化方法在计算效率与稳定性上更优，同时通过局部优势率表示比MF-REINFORCE等离散时间方法更适合含控制分布依赖的扩展MFC问题。"
method_debate_generated_at: "2026-09-02T05:54:28.616505Z"
