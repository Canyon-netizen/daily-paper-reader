...
method_pros_cons: {"SPARK": {"pros": ["提供从诊断到干预的完整潜在推理流水线", "基于冻结模型，无需参数更新", "将潜在剖面转化为可操作的选择机制", "在MATH-500上一致提升Qwen3系列性能"], "cons": ["主要在Qwen3系列验证，跨模型泛化待考", "依赖前向推理中潜在信号，计算开销需评估", "需要构建受控程序推理套件FRONTIER-4.5K"]}, "SPARK-Steering": {"pros": ["从活跃锚点提取方向，针对性干预欠激活样本", "推理时介入，不更新模型参数", "Qwen3-4B在MATH-500从82.0%升至84.6%", "Qwen3-8B在MATH-500从82.4%升至85.6%"], "cons": ["仅测试Qwen3系列，未覆盖其他模型族", "需层选择与方向注入，增加推理复杂度", "仅前向基准剖面，干预范围有限"]}, "Length-controlled susceptibility": {"pros": ["有效分离输入长度效应与推理激活", "解决原始易感性在程序推理中被长度混淆的问题", "残差信号可正负区分激活程度"], "cons": ["依赖相似长度输入基线估计的准确性", "在长度分布外的问题上稳定性未知"]}, "Benchmark evaluation": {"pros": ["广泛使用的标准评估方式，简单直观", "可定位行为能力边界，绘制精度曲线"], "cons": ["仅在输出层面观察，无法揭示失败内部原因", "无法区分能力缺失与激活失败两种情况"]}, "Generic activation-steering methods": {"pros": ["不更新参数即可干预推理过程", "可应用于冻结模型，灵活性较好"], "cons": ["施加全局方向，不诊断哪些样本需要干预", "缺乏针对性，可能损害简单案例性能"]}, "Prompting methods": {"pros": ["实现简单，无需额外训练成本", "部署门槛低，易于应用到各类模型"], "cons": ["将模型视为黑盒，难以精确干预内部状态", "对推理失败的针对性修复能力有限"]}, "Fine-tuning": {"pros": ["可直接提升模型对应任务的能力", "可针对特定能力缺口进行优化"], "cons": ["改变模型参数，模糊冻结模型原有状态", "训练成本高，且可能引入新偏差"]}}
method_comparison: "SPARK通过潜在空间的易感性诊断弥补了基准评估仅看输出的不足，相比通用激活引导方法实现了样本级针对性干预，且不似微调需要更新参数；长度控制易感性是其区分输入尺度与推理激活的关键创新，而SPARK-Steering则将诊断信号转化为冻结模型上的轻量级测试时干预。"
method_debate_generated_at: "2026-09-02T05:50:40.390801Z"
