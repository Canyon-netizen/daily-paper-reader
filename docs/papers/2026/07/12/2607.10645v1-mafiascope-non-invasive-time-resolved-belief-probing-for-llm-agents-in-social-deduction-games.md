...
method_pros_cons: {"MafiaScope": {"pros": ["非侵入式探针：答案不进入游戏上下文", "时间密集：每次发言后均探针，轨迹精细", "自评分机制：与引擎隐藏角色真相对比", "开源引擎+可视化器+200+跨模型游戏"], "cons": ["仅行为级探针，不解析隐藏表征", "依赖LLM自报告，自省能力存疑", "案例研究仅用DeepSeek单一模型", "局限于Mafia一种游戏类型"]}, "DeepRole": {"pros": ["维护角色分配的后验信念分布", "内置专用规划器，推理明确"], "cons": ["仅限Avalon游戏，通用性受限", "需专用规划系统，依赖定制架构", "不适用于闭源API模型"]}, "ToMATO": {"pros": ["每句发言均让LLM语言化心智状态", "基于角色扮演，覆盖多轮对话"], "cons": ["结果冻结为静态QA基准", "非交互式，无法捕捉动态博弈"]}, "MultiMind": {"pros": ["上下文建模各玩家对己方的怀疑", "与MafiaScope社交地图探针最接近"], "cons": ["角色关系与MafiaScope相反", "探针影响后续行为，具有侵入性"]}, "静态ToM基准(ToMi/FANToM/Hi-ToM等)": {"pros": ["评测标准化，体系成熟", "便于大规模自动化评分"], "cons": ["静态故事/脚本，非对抗交互", "结论脆弱且存在争议", "无法展示信念在交互中的演化"]}}
method_comparison: "MafiaScope是面向社交推理游戏的非侵入式时序信念测量工具，与DeepRole（专用规划）和MultiMind（侵入式上下文建模）形成对比，相较ToMATO（静态冻结）和静态ToM基准（无动态博弈），MafiaScope在时间分辨率与行为真实性上具有显著优势。"
method_debate_generated_at: "2026-09-02T05:46:26.344461Z"
