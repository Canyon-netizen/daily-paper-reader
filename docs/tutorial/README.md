...
左侧侧边栏按时间倒序组织所有论文，方便按日期浏览：

- **Daily Papers**：按日期分组，每一天是一个可折叠/展开的列表。默认仅展开最新一天；点击日期标题（如 `2026-06-18`）即可切换展开/收起，状态会保存在浏览器中。
- **Authors**：收录 2 篇及以上论文的作者列表，**默认收起**。点击 "Authors" 标题即可展开或折叠整个作者分组；状态会保存到浏览器 localStorage，下次访问自动恢复你的选择。
- **Conference**：按会议分组的论文列表，同样可折叠/展开。

折叠列表时，标题右侧会有一个小三角箭头（`▸` 表示收起，`▾` 表示展开）。点击标题文字或箭头即可切换状态。
...
5. 下载仓库里的脚本，并导入到 Zotero 中  
   脚本链接：
   <a href="docs/tutorial/zotero-actions-backup.yml" data-no-router download="zotero-actions-backup.yml">下载 zotero-actions-backup.yml</a>
